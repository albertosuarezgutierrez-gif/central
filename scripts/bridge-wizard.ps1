# ============================================================
# ia.rest · Bridge Wizard v5 — con captura de MAC
# ============================================================
param(
...
# Obtener MAC de una IP via ARP
function Get-MacAddress($ip) {
    try {
        # Ping para poblar ARP cache
...
if ($impresorasExistentes -gt 0) {
    ...
    if ($resp2 -ne "s" -and $resp2 -ne "S") {
        Start-Process "$API/owner?setup=1"; Start-Sleep 2; exit 0
    }
} else {
...
Write-INFO "Escaneando red... (~30 segundos)"
Write-Host ""
...
$pool.Close(); Write-Progress -Activity "Escaneando..." -Completed; Write-Host ""
...
if ($found.Count -eq 0) {
    ...
    } else { Write-INFO "Puedes añadirlas desde /owner → Hardware."; Read-Host "  Enter"; exit 0 }
}
...
foreach ($ip in $found) {
    ...
    if ($mac) {
        ...
        Write-WARN "  MAC no disponible (DHCP reserva recomendada)"
    }
    ...
    try {
        ...
        Write-OK "Registrada: $nombre ($ip)$(if($mac){' · MAC guardada'})"
        [void]$registradas.Add(@{id=$r.id; nombre=$nombre; ip=$ip; mac=$mac})
        $n++
...
Write-INFO "· Pulsa TEST en el panel para identificar cada una"
Write-INFO "· Crea los flujos de trabajo para asignar secciones"
Write-Host ""
