...
//
// Reglas:
// - Una "entrada" = bloque que empieza en columna 0 con `- **` hasta la siguiente.
// - La fecha de una entrada sale SOLO de su primera línea (la cabecera en negrita);
//   las fechas citadas en el cuerpo mienten (leyes, facturas, sesiones antiguas).
//   Si la cabecera no trae fecha (o trae dd/mm sin año), se hereda/completa con la
//   entrada de arriba: el archivo es cronológico descendente, así que a nivel de
//   mes la herencia es exacta.
...
const primeraEntrada = lineas.findIndex((l) => l.startsWith('- **'));
if (primeraEntrada === -1) {
...
for (const entrada of entradas) {
  const m = entrada[0].match(RE_FECHA);
  let mes = null;
  if (m) {
