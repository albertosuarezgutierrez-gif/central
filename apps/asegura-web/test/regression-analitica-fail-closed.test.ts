...
test('layout.tsx no tiene NINGÚN script de Cookiebot, PostHog o GA4 suelto', () => {
  // Se busca la FORMA de un <script> real (id/src), no la palabra "Cookiebot"/
  // "PostHog"/"GA4" a secas — esas palabras aparecen legítimamente en comentarios
  ...
  assert.ok(!/googletagmanager\.com\/gtag\/js/.test(layout), 'el script de GA4 no debe cargarse desde layout.tsx directamente')
})

test('Analitica.tsx arranca PostHog Y GA4 SOLO dentro de la comprobación de consentimiento', () => {
  ...
  assert.ok(/cargarGa4/.test(analitica), 'GA4 debe cargarse con el adaptador del paquete, no un gtag suelto')
  // El guard que decide si se monta el banner NO puede depender solo de
  // POSTHOG_KEY: eso dejaría a GA4 sin banner que lo arranque el día que
  // falte la clave de PostHog en Vercel (ver comentario en el propio componente).
  assert.ok(!/if \(!POSTHOG_KEY\) return/.test(analitica), 'el guard de montaje no debe depender solo de POSTHOG_KEY')
})
