...
test('la página de ramo monta el widget con el ramo como origen medido', () => {
  ...
  assert.match(comp, /`ramo_ventana_\$\{ramoFijo \?\? 'portada'\}`/)
  assert.match(comp, /origen=\{origen\}/)
  const home = readFileSync(join(import.meta.dirname, '..', 'app', 'page.tsx'), 'utf8')
  assert.match(home, /<VentanaRenovacion\b[\s\S]*?opciones=/, 'la portada monta el widget con selector de ramo')
