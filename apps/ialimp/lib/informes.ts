...
export interface GenerarInformeResult {
  ok: true
  sesiones: number
  resumen_ia: string
...
// Genera (o regenera) el informe mensual de un cliente con el `empresa_id`
// EXPLÍCITO. Fuente única de la lógica: la usan tanto la ruta
// `POST /api/admin/informes/generar` (usuario logueado → empresa_id de la
// sesión) como el cron `informes/cron` (bucle por empresa, empresa_id explícito
// → sin sub-fetch HTTP que perdía la cookie y devolvía 401 silencioso).
export async function generarInforme(
  { empresa_id, cliente_id, periodo, enviar_email = false }: GenerarInformeParams
...
<div style="margin-top:32px;padding:16px;background:#f8fafc;border-radius:8px;font-size:12px;color:#64748b">
  ...
  return { ok: true, sesiones: sesiones.length, resumen_ia, email_enviado }
}
