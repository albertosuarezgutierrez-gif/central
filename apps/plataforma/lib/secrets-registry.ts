...
export const SECRETS_REGISTRY: SecretEntry[] = [
  ...
  { name: 'GITHUB_TOKEN', tipo: 'api-externa', proposito: 'Lee y commitea la landing de housesevillana (apps/housesevillana del repo central) para el agente SEO.', verticales: ['sivra', 'plataforma'], dondeVive: 'vercel-proyecto', proyecto: 'sivra + plataforma', editable: true, vercelProject: 'sivra', vercelProjects: ['plataforma'], nota: 'Runtime env que leen seo-landing.ts de sivra Y plataforma (mismo valor en ambos). 🚨 PAT fine-grained con Repository access = albertosuarezgutierrez-gif/central y Contents: Read and write — la landing vive en el MONOREPO desde el 12/08/2026, no en el antiguo repo externo house-sevillana-landing; con el scope viejo el GET cuela (repo público) pero el commit da 403. Compruébalo sin esperar al lunes con el botón "Probar acceso a GitHub" de /sivra/seo. NO confundir con el GITHUB_TOKEN auto-inyectado en GitHub Actions. La regla "usar GH_PAT" aplica al código de ia-rest, no a estas dos rutas SEO ya estandarizadas. 🚨 «Contents: Read and write» de aquí arriba es lo que el agente SEO NECESITA, no el inventario de permisos del token: los tres PAT con acceso a central llevan además «Pull requests: Read and write» (comprobado en el panel de GitHub el 07/09/2026). Se dice porque ese mismo día se leyó esta nota como si fuera la lista completa y se afirmó en un PR que al token le faltaba el permiso de PRs — falso, y un no-op de trabajo para Alberto. Al describir un secreto aquí, distingue «lo que hace falta para X» de «lo que tiene».' },
  { name: 'GSC_SA_CLIENT_EMAIL', tipo: 'api-externa', proposito: 'Cuenta de servicio de Google con acceso (restringido) a la propiedad sc-domain:grupoasegura.es de Search Console. Es el `client_email` del JSON de la cuenta. La leen el cron SEO de la correduría (lunes) y `/api/internal/gsc` (consulta a demanda).', verticales: ['plataforma'], dondeVive: 'vercel-proyecto', proyecto: 'plataforma', editable: true, vercelProject: 'plataforma' },
  { name: 'GSC_SA_PRIVATE_KEY', tipo: 'api-externa', proposito: 'Clave privada (`private_key` del JSON de la cuenta de servicio) con la que el cron SEO y `/api/internal/gsc` firman el JWT para Search Console. Se pega tal cual, con sus \\n escapados: el cargador los tolera.', verticales: ['plataforma'], dondeVive: 'vercel-proyecto', proyecto: 'plataforma', editable: true, vercelProject: 'plataforma' },
  { name: 'POSTHOG_PERSONAL_API_KEY', tipo: 'api-externa', proposito: 'Personal API key de PostHog EU (solo lectura, proyecto 266897 «Grupo ASegura») para leer visitas medidas por HogQL desde el cron SEO. NO es la clave pública phc_ del navegador.', verticales: ['plataforma'], dondeVive: 'vercel-proyecto', proyecto: 'plataforma', editable: true, vercelProject: 'plataforma' },
  { name: 'NEXT_PUBLIC_COOKIEBOT_ID', tipo: 'api-externa', proposito: 'OBSOLETA (14/09/2026): identificador del dominio en Cookiebot, usado por asegura-web para gatear PostHog. Sustituido por un CMP propio (@central/core-consent + vanilla-cookieconsent, sin credencial de terceros) — ver docs/superpowers/plans/2026-09-14-consentimiento-unificado.md. Ya no la usa ninguna app del monorepo. Puede eliminarse de Vercel (si llegó a ponerse) y de un eventual panel de Cookiebot.', verticales: [], dondeVive: 'vercel-proyecto', proyecto: 'asegura-web', obligatoria: false },
  { name: 'SERPER_API_KEY', tipo: 'api-externa', proposito: 'Búsqueda web en Google. Retirada del cron de mercado el 24/08/2026 (créditos agotados) — el código de SIVRA sigue ahí, inerte, por si algún día se dispara a mano. Usada del 08 al 14/09/2026 por el cron SEO de la correduría; retirada de ahí también (decisión de Alberto: política «todo lo que pueda ir por OpenRouter, va por OpenRouter» + SERP-position-tracking innecesario en esta fase). Ya no la usa ninguna app en producción.', verticales: ['sivra'], dondeVive: 'vercel-proyecto', proyecto: 'sivra', editable: true, vercelProject: 'sivra' },
