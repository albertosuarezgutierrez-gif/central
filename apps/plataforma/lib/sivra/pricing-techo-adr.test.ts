...
import {
  aplicarTechoAdr, HOLGURA_ADR, MIN_NOCHES_ADR, UMBRAL_EVENTO, MIN_COMPS_EVENTO_MEDIDO,
...
test('una fecha de EVENTO con mercado MEDIDO no se juzga con el histórico del mes', () => {
  // 20/02/2027, Maratón de Sevilla: el mercado sube x2,5 y el motor le sigue bien.
  const r = aplicarTechoAdr({ objetivo: 1193, adrBase: 400, nochesMuestra: 210, factorEvento: 1.5, compsFiablesFecha: 10 })
...
test('el umbral de evento es el mismo que usa el resto del motor', () => {
  assert.equal(aplicarTechoAdr({ objetivo: 500, adrBase: 100, nochesMuestra: 100, factorEvento: UMBRAL_EVENTO, compsFiablesFecha: 10 }).motivo, 'evento')
...
// ── Evento SIN mercado medido: techo escalado por su factor (23/09/2026) ──────────────────────

test('evento sin mercado medido: el techo se escala por el factor, no desaparece', () => {
  // Luxury Busto, 08/11/2026 (Sevilla-Alavés ×1,35, sin comps de su fecha): pedía 200€ con los
  // días normales a 75€. Con ADR base 60 el techo es 60 × 1,30 × 1,35 = 105.
  const r = aplicarTechoAdr({ objetivo: 200, adrBase: 60, nochesMuestra: 134, factorEvento: 1.35, compsFiablesFecha: 0 })
  assert.equal(r.motivo, 'aplicado')
  assert.equal(r.techo, Math.round(60 * HOLGURA_ADR * 1.35))
  assert.equal(r.objetivo, r.techo)
  assert.equal(r.escaladoEvento, 1.35)
})

test('evento sin mercado medido: con el umbral de comps justo por debajo sigue escalado', () => {
  const r = aplicarTechoAdr({ objetivo: 500, adrBase: 100, nochesMuestra: 100, factorEvento: 2,
    compsFiablesFecha: MIN_COMPS_EVENTO_MEDIDO - 1 })
  assert.equal(r.motivo, 'aplicado')
  assert.equal(r.techo, Math.round(100 * HOLGURA_ADR * 2))
})

test('evento con comps sin poder leer (null): se deja al mercado, no se recorta a ciegas', () => {
  for (const comps of [null, undefined]) {
    const r = aplicarTechoAdr({ objetivo: 500, adrBase: 100, nochesMuestra: 100, factorEvento: 1.5, compsFiablesFecha: comps })
    assert.equal(r.motivo, 'evento', `comps=${comps}`)
    assert.equal(r.objetivo, 500)
  }
})

test('evento escalado que no llega al techo: no muerde', () => {
  const r = aplicarTechoAdr({ objetivo: 150, adrBase: 100, nochesMuestra: 100, factorEvento: 1.5, compsFiablesFecha: 0 })
  assert.equal(r.motivo, 'no_muerde')
  assert.equal(r.objetivo, 150)
})

test('evento escalado tampoco perfora el suelo', () => {
  const r = aplicarTechoAdr({ objetivo: 90, adrBase: 40, nochesMuestra: 100, factorEvento: 1.2, compsFiablesFecha: 0, suelo: 80 })
  assert.equal(r.motivo, 'suelo_manda')
  assert.equal(r.objetivo, 90)
})

test('día normal: el techo no se escala aunque lleguen comps', () => {
  const r = aplicarTechoAdr({ objetivo: 500, adrBase: 100, nochesMuestra: 100, factorEvento: 1, compsFiablesFecha: 0 })
  assert.equal(r.techo, Math.round(100 * HOLGURA_ADR))
  assert.equal(r.escaladoEvento, undefined)
})
