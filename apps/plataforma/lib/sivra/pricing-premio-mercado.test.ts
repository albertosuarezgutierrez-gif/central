...
test("premio: Karol G (mercado fecha 931, mes ~130) dispara y ancla a la fecha, no al mes", () => {
  ...
  const premio = premioMercadoFecha({ medFechaGuest: 931, comps: 10, normalBase: 112, markup: MARKUP, dqFactor: 1 })
...
test("premio: Feria (mercado fecha 424, mes ~186) dispara", () => {
  const premio = premioMercadoFecha({ medFechaGuest: 424, comps: 10, normalBase: 160, markup: MARKUP, dqFactor: 1 })
...
test("premio: NO dispara en un finde normal (mercado fecha solo 1.3× la base del mes)", () => {
  ...
  const premio = premioMercadoFecha({ medFechaGuest: 210, comps: 10, normalBase: 150, markup: MARKUP, dqFactor: 1 })
  assert.equal(premio, 0)
})

test("premio: NO dispara con muestra pobre de la fecha (<3 comps)", () => {
  const premio = premioMercadoFecha({ medFechaGuest: 931, comps: 2, normalBase: 112, markup: MARKUP, dqFactor: 1 })
  assert.equal(premio, 0)
})

test("premio: respeta el ajuste demanda/calidad (dqFactor) del motor", () => {
  const premio = premioMercadoFecha({ medFechaGuest: 424, comps: 10, normalBase: 150, markup: MARKUP, dqFactor: 1.05 })
  assert.equal(premio, Math.round((424 * 1.05) / MARKUP))
})

test("premio: umbral configurable (ratio 1.25 captura una fecha 1.3×)", () => {
  const base = premioMercadoFecha({ medFechaGuest: 278, comps: 10, normalBase: 181, markup: MARKUP, dqFactor: 1 })
  assert.equal(base, 0) // con el 1.5 por defecto no dispara
  const conMenor = premioMercadoFecha({ medFechaGuest: 278, comps: 10, normalBase: 181, markup: MARKUP, dqFactor: 1 }, { ratio: 1.25 })
