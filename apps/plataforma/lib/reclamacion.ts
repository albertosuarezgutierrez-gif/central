...
export async function redactarReclamacion(input: ReclamacionInput): Promise<{ asunto: string; cuerpo: string }> {
  const eur = Math.abs(input.importe).toFixed(2)
  const asunto = `Reclamación por cargo duplicado — ${input.comercio} (${eur} €)`
  const plantilla = [
    `Estimados,`,
    ``,
    `He detectado un cargo duplicado de ${eur} € correspondiente a "${input.comercio}",`,
  ...
  try {
    const cuerpo = await aiComplete(
      `Comercio: ${input.comercio}. Importe duplicado: ${eur} €. Fechas: ${input.fechas.join(', ')}.`,
      {
