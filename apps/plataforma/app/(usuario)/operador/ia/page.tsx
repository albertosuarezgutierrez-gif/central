  ...
  return (
    <main style={{ padding: 24, maxWidth: 920 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginTop: 16 }}>
        <section style={{ ...card, flex: 1, minWidth: 260 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr><th style={th}>App</th><th style={th}>Llamadas</th></tr></thead>
            <tbody>
              {r.por_app.map(a => <tr key={a.app}><td style={td}>{a.app}</td><td style={td}>{a.n}</td></tr>)}
              {r.por_app.length === 0 && <tr><td style={td} colSpan={2}>Sin uso este mes</td></tr>}
            </tbody>
          </table>
        </section>
        <section style={{ ...card, flex: 1, minWidth: 260 }}>
      ...
      <section style={{ ...card, marginTop: 16 }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr><th style={th}>Fecha</th><th style={th}>App</th><th style={th}>Tipo</th><th style={th}>Proveedor</th><th style={th}>Estado</th><th style={th}>ms</th></tr></thead>
          <tbody>
            {r.recientes.map((u, i) => (
              <tr key={i}>
            ...
            {r.recientes.length === 0 && <tr><td style={td} colSpan={6}>Aún no hay llamadas registradas.</td></tr>}
          </tbody>
        </table>
      </section>
