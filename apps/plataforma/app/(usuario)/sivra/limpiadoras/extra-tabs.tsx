...
function Spinner() { return <div style={{textAlign:'center',padding:32,color:'var(--muted)'}}>Cargando...</div> }
...
export function TabChecklists() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const r = await fetch(`/api/sivra/limpiadoras/checklist?property_id=${selProp}`)
  ...
  async function addItem() {
    ...
    setSaving(true)
    await fetch('/api/sivra/limpiadoras/checklist',{method:'POST',headers:{'Content-Type':'application/json'},
  ...
  async function saveEdit() {
    if (!editId) return
    setSaving(true)
    await fetch('/api/sivra/limpiadoras/checklist',{method:'PUT',headers:{'Content-Type':'application/json'},
  ...
  async function deleteItem(id: string) {
    ...
    await fetch('/api/sivra/limpiadoras/checklist',{method:'DELETE',headers:{'Content-Type':'application/json'},
  ...
  async function uploadRef(itemId: string, file: File) {
    ...
    const r = await fetch('/api/limpiadoras/upload-photo',{method:'POST',body:form})
    const d = await r.json()
    if (d.url) {
      await fetch('/api/sivra/limpiadoras/checklist',{method:'PUT',headers:{'Content-Type':'application/json'},
  ...
  return (
    <div>
      ...
      <div style={{marginBottom:20}}>
        {items.length===0&&<div style={{color:'var(--muted)',textAlign:'center',padding:24}}>Sin ítems configurados</div>}
        {items.map((item,idx)=>(
          <div key={item.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'12px 14px',marginBottom:8}}>
            {editId===item.id ? (
              <div>
                ...
                <div style={{display:'flex',gap:8}}>
                  ...
                  <button onClick={()=>setEditId(null)}
                    style={{background:'none',border:'1px solid var(--border)',borderRadius:8,padding:'7px 14px',fontSize:13,cursor:'pointer',color:'var(--text)'}}>
                    Cancelar
                  </button>
                </div>
              </div>
              ...
              <div style={{display:'flex',alignItems:'flex-start',gap:10}}>
                <div style={{display:'flex',flexDirection:'column',gap:2}}>
                  <button onClick={()=>move(item,'up')} disabled={idx===0}
                    style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:14,padding:'0 4px',opacity:idx===0?0.3:1}}>▲</button>
                  <button onClick={()=>move(item,'down')} disabled={idx===items.length-1}
                    style={{background:'none',border:'none',cursor:'pointer',color:'var(--muted)',fontSize:14,padding:'0 4px',opacity:idx===items.length-1?0.3:1}}>▼</button>
                </div>
                <div style={{flex:1}}>
                ...
                <div style={{display:'flex',gap:6,alignItems:'center'}}>
                  ...
                  <button onClick={()=>uploadRefs.current[item.id]?.click()}
                    style={{background:'none',border:'1px solid var(--border)',borderRadius:7,padding:'4px 8px',fontSize:11,cursor:'pointer'}}
      ...
      <div style={{background:'var(--surface)',borderRadius:12,padding:16,border:'1px dashed var(--border)'}}>
        <div style={{fontWeight:700,fontSize:13,marginBottom:12,color:'var(--text)'}}>+ Nuevo ítem</div>
...
export function TabInformes() {
  ...
  useEffect(()=>{
    fetch('/api/sivra/limpiadoras/usuarios').then(r=>r.json()).then(d=>{
  ...
  async function loadPreview() {
    ...
    const r = await fetch(`/api/sivra/limpiadoras/informe?lid=${selLimp}&from=${desde}&to=${hasta}`)
  ...
  function openInforme() {
    if (!selLimp) return
    const desde = firstOfMonth(monthOffset); const hasta = lastOfMonth(monthOffset)
    window.open(`/api/sivra/limpiadoras/informe?lid=${selLimp}&from=${desde}&to=${hasta}`,'_blank')
  }
    ...
    <div>
        ...
        <div className="extra-informe-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
          <div>
            ...
            <select value={selLimp} onChange={e=>setSelLimp(e.target.value)}
              style={{width:'100%',border:'1px solid var(--border)',borderRadius:8,padding:'9px 10px',fontSize:14,background:'var(--surface)',color:'var(--text)'}}>
              {limpiadoras.map(l=><option key={l.id} value={l.id}>{l.nombre}</option>)}
            </select>
          </div>
          <div>
            <div style={{fontSize:11,color:'var(--muted)',marginBottom:5,fontWeight:600}}>Período</div>
            <div style={{display:'flex',alignItems:'center',gap:6}}>
              <button onClick={()=>setMonthOffset(o=>o-1)}
                style={{border:'1px solid var(--border)',borderRadius:8,padding:'7px 12px',background:'var(--surface)',cursor:'pointer',fontSize:14,color:'var(--text)'}}>‹</button>
              <span style={{flex:1,textAlign:'center',fontSize:13,fontWeight:600,textTransform:'capitalize',color:'var(--text)'}}>{mesLabel}</span>
              <button onClick={()=>setMonthOffset(o=>o+1)} disabled={monthOffset>=0}
                style={{border:'1px solid var(--border)',borderRadius:8,padding:'7px 12px',background:'var(--surface)',cursor:'pointer',fontSize:14,color:'var(--text)',opacity:monthOffset>=0?0.4:1}}>›</button>
            </div>
          </div>
        </div>
        <div style={{display:'flex',gap:10}}>
          <button onClick={loadPreview} disabled={loading||!selLimp}
            style={{flex:1,background:'var(--border)',color:'var(--text)',border:'none',borderRadius:9,padding:'10px 0',fontWeight:600,fontSize:14,cursor:'pointer'}}>
            👁 Vista previa
          </button>
      ...
      {preview&&!loading&&(
        <div style={{border:'1px solid var(--border)',borderRadius:12,overflow:'hidden'}}>
          <div style={{background:'var(--surface)',padding:'10px 14px',fontSize:12,color:'var(--muted)',fontWeight:600}}>
...
export function TabFacturacion() {
  ...
  const load = useCallback(async()=>{
    ...
    const [r1,r2,r3] = await Promise.all([
      ...
      fetch('/api/sivra/limpiadoras/tarifas'),
      fetch('/api/sivra/limpiadoras/facturacion'),
    ])
  ...
  async function updateEstado(id: string, estado: string) {
    await fetch('/api/sivra/limpiadoras/facturacion',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,estado})})
    load()
  }

  async function saveTarifa() {
    if(!newTarifa.limpiadora_id||!newTarifa.importe) return
    await fetch('/api/sivra/limpiadoras/tarifas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(newTarifa)})
    ...
    <div>
      ...
      {view==='tarifas'&&(
        <div>
          ...
          {tarifas.map(t=>(
            <div key={t.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:10,padding:'12px 14px',marginBottom:8,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div>
        ...
        <div>
          <div style={{background:'#f0fdf4',border:'1px solid #86efac',borderRadius:12,padding:16,marginBottom:16}}>
            ...
            <button onClick={generateFactura} disabled={generating||!gen.limpiadora_id||!gen.desde||!gen.hasta}
              style={{background:'#1B4332',color:'#fff',border:'none',borderRadius:9,padding:'10px 22px',fontWeight:700,fontSize:14,cursor:'pointer',opacity:generating?0.6:1}}>
              {generating?'Generando...':'⚡ Generar factura'}
            </button>
          </div>
          {facturas.length===0&&<div style={{textAlign:'center',color:'var(--muted)',padding:32}}>Sin facturas generadas</div>}
          {facturas.map(f=>{
            const l=limpiadoras.find(x=>x.id===f.limpiadora_id)
            const est=ESTADOS[f.estado]||ESTADOS.borrador
            return (
              <div key={f.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:16,marginBottom:10}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                  <div>
                    <div style={{fontWeight:700,fontSize:15,color:'var(--text)'}}>{f.numero}</div>
                    <div style={{fontSize:12,color:'var(--muted)'}}>{l?.nombre||'?'} · {f.periodo_inicio} → {f.periodo_fin}</div>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  <button onClick={()=>window.open(`/api/sivra/limpiadoras/informe?lid=${f.limpiadora_id}&from=${f.periodo_inicio}&to=${f.periodo_fin}`,'_blank')}
                    style={{border:'1px solid var(--border)',borderRadius:8,padding:'6px 12px',fontSize:12,cursor:'pointer',background:'var(--surface)',color:'var(--text)'}}>
                    🖨️ Informe PDF
                  </button>
