...
export default function PricingAutoPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const sr = await fetch("/api/sivra/pricing/settings", { cache: "no-store" })
  ...
  const save = async (id: string) => {
    ...
    try {
      const r = await fetch("/api/sivra/pricing/settings", {
  ...
  const apply = async (id: string, dryRun: boolean) => {
    ...
    setBusy(id); setMsg((m) => ({ ...m, [id]: "" }))
    try {
      const r = await fetch(`/api/sivra/pricing/apply?property=${id}&dryRun=${dryRun}`, { method: "POST" })
      const j = await r.json()
  ...
  const restore = async (id: string) => {
    ...
    setBusy(id); setMsg((m) => ({ ...m, [id]: "" }))
    try {
      const r = await fetch(`/api/sivra/pricing/restore?property=${id}`, { method: "POST" })
      const j = await r.json()
  ...
  const togglePause = async () => {
    const next = !paused
    setPaused(next)
    await fetch("/api/sivra/pricing/config", {
  ...
  const loadHist = async (id: string) => {
    ...
    const j = await fetch(`/api/sivra/pricing/historial?property=${id}&limit=40`).then(x => x.json()).catch(() => null)
  ...
  const runPilot = async () => {
    setPilotBusy(true)
    try {
      await fetch("/api/sivra/pricing/pilot-track", { method: "POST" }).catch(() => null)
      await load()
