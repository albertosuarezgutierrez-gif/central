 ...
 */
export default function AccionesContacto({ telefono, email, ilegible, quien, mensaje, onWhatsapp, tam = 'sm' }: {
  ...
  mensaje?: string | null
  /** Se dispara al pulsar WhatsApp, sin bloquear la apertura. Ver `onAbrir` de `BotonWhatsapp`. */
  onWhatsapp?: () => void
  tam?: 'sm' | 'md'
}) {
  ...
  return (
    ...
    <span style={{ display: 'inline-flex', gap: 2, alignItems: 'center', verticalAlign: 'middle', alignSelf: 'center' }}>
      ...
      {telefono && !ilegible && <BotonWhatsapp telefono={telefono} mensaje={mensaje} onAbrir={onWhatsapp} compacto />}
      {a.email && (
