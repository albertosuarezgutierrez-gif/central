  ...
  return (
    <div>
      <div style={{ position: 'relative', height: '20px', borderRadius: '6px', overflow: 'hidden', display: 'flex', marginTop: '10px' }}>
        ...
        {base > 0 && <div style={{ position: 'absolute', top: 0, bottom: 0, width: '3px', background: 'var(--text)', left: `${pct}%` }} />}
      </div>
      {base > 0 && (
        <div style={{ position: 'relative', height: '18px', marginTop: '2px' }}>
          <span style={{ position: 'absolute', left: `${pct}%`, transform: 'translateX(-50%)', fontSize: '10px', fontWeight: 700, color: 'var(--text)', whiteSpace: 'nowrap' }}>▲ {eur(base)}</span>
        </div>
      )}
