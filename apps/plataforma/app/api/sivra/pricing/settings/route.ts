...
// calcularía el motor con esos parámetros — la misma cadena que /api/sivra/pricing/apply.
//
...
export async function GET() {
  ...
  const properties = props.map((p) => {
    ...
    if (g && g.prices.length > 0) {
      ...
      // Motor compartido (lib/sivra/pricing-engine) → mismo recomendado que recommend y el agente.
