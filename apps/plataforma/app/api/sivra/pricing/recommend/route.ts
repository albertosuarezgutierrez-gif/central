...
// GET /api/sivra/pricing/recommend
//
...
// (ocupación). El cálculo vive en `lib/sivra/pricing-engine.ts` (FUENTE ÚNICA, compartida
...
export async function GET() {
  ...
  const recommendations = props.map(p => {
    ...
    // min/max del propietario sobre el huésped (comportamiento histórico).
    let price = eng.guest
