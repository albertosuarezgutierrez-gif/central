...
Extrae apartamentos con precios reales en euros. SOLO JSON.`
  ...
  try {
    const txt   = (await chatConDirector([{ role: "user", content: prompt }], { app: "plataforma", endpoint: "mercado-search", system, maxTokens: 800, temperature: 0.1 })).text
...
async function searchPortal(
  ...
  try {
    ...
    console.error(`[sivra/mercado/search] error ${portal}:`, e)
