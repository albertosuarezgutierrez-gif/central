...
export async function GET() {
  ...
  try {
    ...
    for (const sc of scenarios) {
      const allApts = Object.values(byKey).filter((v: any) => v.scenario === sc).flatMap((v: any) => v.apartments)
      const prices = allApts.map((a: any) => a.price_night).filter(Boolean).sort((a: number, b: number) => a - b)
      if (prices.length > 0) {
        byKey[`${sc}_all`] = {
          scenario: sc, portal: 'all',
          stats: {
            min: prices[0], p25: percentile(prices, 0.25), p50: percentile(prices, 0.50),
            p75: percentile(prices, 0.75), max: prices[prices.length - 1],
            avg: Math.round(prices.reduce((s: number, p: number) => s + p, 0) / prices.length),
            count: prices.length,
          }
        }
