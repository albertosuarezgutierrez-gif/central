...
import {
  accionOportunidadAsegura,
  interpretarOportunidad,
  interpretarOportunidadesCliente,
  oportunidadAsegura,
  oportunidadesClienteAsegura,
 ...
 *   GET  ?id=           → una oportunidad
 *   GET  ?clienteId=    → las de ese cliente (tarjeta de la ficha)
 *   POST { id, accion, motivo?, detalle?, competidor?, primaCompetidor?, aparcadaHasta?, polizaGanadaId? }
 *   POST { accion:'crear', clienteId, ramo, ... }  ·  POST { accion:'editar', id, ... }
 */
export async function GET(req: NextRequest) {
  ...
  if (clienteId !== '') {
    const r = await oportunidadesClienteAsegura(clienteId)
    return NextResponse.json(interpretarOportunidadesCliente(r.status, r.json))
  }
...
export async function POST(req: NextRequest) {
  ...
  const crear = body?.accion === 'crear' && typeof body.clienteId === 'string'
  if (!body || (!crear && typeof body.id !== 'string') || typeof body.accion !== 'string') {
    return NextResponse.json({ estado: 'invalido', motivo: 'Faltan id (o clienteId al crear) y accion.' }, { status: 422 })
  }
