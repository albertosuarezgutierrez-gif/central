...
import { disponible, crearRequisition } from '@/lib/gocardless'
...
export async function POST(req: NextRequest) {
  ...
  if (!disponible()) return NextResponse.json({ error: 'GoCardless sin configurar (GOCARDLESS_SECRET_ID/KEY)' }, { status: 503 })
  ...
  try {
    const requisition = await crearRequisition(body.institutionId, `${origin}/api/banca/psd2/callback`, randomUUID())
    await prisma.$executeRaw`
      INSERT INTO conexiones_banco (cuenta_id, sociedad_id, institution_id, institution_nombre, requisition_id, estado)
      VALUES (${session.id}::uuid, ${body.sociedadId}::uuid, ${body.institutionId}, ${body.institutionNombre ?? null}, ${requisition.id}, 'pendiente')
    `
