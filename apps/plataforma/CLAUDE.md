...
| `EMPRESAS_INVITADO_TOKEN` | **Token de acceso INVITADO a «Empresas»** (para que Pablo pruebe el módulo sin cuenta). Secreto que concede acceso → **SIN fallback literal**: si no está definido, el acceso invitado queda DESACTIVADO. El enlace es `…/invitado/empresas?token=<valor>` (la 1ª visita fija cookie `empresas_invitado`); `lib/empresas-acceso.ts::accesoEmpresas` acepta sesión O este token en las rutas `/api/empresas/*` **salvo el enriquecimiento (POST), que es SOLO sesión** (gasta dinero). El invitado no ve los botones «Enriquecer» ni «Actualizar BORME». Para revocar: cambia/borra la env. |
