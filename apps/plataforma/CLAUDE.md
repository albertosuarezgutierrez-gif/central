...
- [x] **🏠 Resumen + Banca FUSIONADOS → Inicio único `💶 Dinero | 🏢 Negocios` (16/07/2026, Fase 2):**
  ...
  **🏠 Inicio** (`UserSidebar.tsx`, fusiona Resumen+Banca). **Ficha de movimiento (PR2):** tocar el concepto de
  ...
  Ambos segmentos se renderizan en SSR (el inactivo con `display:none`). ⚠️ **La sección de abajo "Home
  `/dashboard` = RESUMEN" describe el estado ANTERIOR** (dashboard ya no renderiza nada, solo redirige); su
  lógica de widgets vive ahora en `NegociosResumen`. ⚠️ Coste: `/banca` carga también el holding en cada request
  (NegociosResumen no es perezoso) — candidato a lazy-load.
