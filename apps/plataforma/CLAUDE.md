...
| `DIRECTOR_MODO` | **🟢 En producción `activo` desde el 10/07/2026** (la semana de sombra se acortó a 1 día por decisión de Alberto). `sombra` (el Director decide y se REGISTRA en `ai_usos` pero se sirve con el modelo por defecto) · `activo` (enruta de verdad). Opcionales: `DIRECTOR_MODEL` (modelo barato que decide, default `deepseek/deepseek-chat`), `DIRECTOR_USAR_FLOOR` (sufijo `:floor` = proveedor más barato; **opt-in desde el 11/07/2026 (PR #828), default OFF** — enrutaba a proveedores `:free`/rate-limited y causaba 429 intermitentes en el primario; `true` lo reactiva), `DIRECTOR_MAX_PRECIO_OUT` (techo USD/M del cron, default 20). |
| `PASARELA_MODELO_SEGURO` | Red de seguridad ante 429 (11/07/2026, PR #829): si OpenRouter falla, la pasarela reintenta UNA vez con este modelo de un solo tiro antes de caer a la cadena clásica NIM/Groq/Gemini/Kimi. Default `meta-llama/llama-3.3-70b-instruct`. No toca las decisiones del Director. |
