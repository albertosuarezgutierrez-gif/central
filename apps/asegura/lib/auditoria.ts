...
// Todavía NO guarda el antes/después de cada campo: eso es la pieza (c).
//
...
export function auditado<C>(manejador: Manejador<C>): (req: Request, ctx: C) => Promise<Response> {
  return async (req, ctx) => {
    ...
    let estado = 500
    try {
      const res = await manejador(req, ctx)
      estado = res.status
      return res
    } finally {
      if (autorizado) await registrarEscritura(req, copia, estado, Date.now() - t0)
    }
  }
}

async function registrarEscritura(req: Request, copia: Request | null, estado: number, duracionMs: number): Promise<void> {
  try {
    ...
    await prismaAsegura().$executeRaw`
      insert into auditoria (actor_tipo, actor_id, metodo, ruta, estado_http, ids, duracion_ms)
      values (${actor.tipo}, ${actorId}, ${req.method}, ${ruta}, ${estado}, ${ids}::jsonb, ${duracionMs})`
  } catch (e) {
