...
test('solo escribe NUESTROS campos, y sin pisar un enlace que ya exista', () => {
  ...
  assert.doesNotMatch(src, /update polizas\s+set[^`]*\bestado\s*=/, 'el estado de la póliza es de CIMA')
})
...
test('🚨 la oportunidad se gana sola SOLO con una candidata del mismo cliente y la misma matrícula', () => {
  ...
  assert.match(f, /from cand where n = 1/, 'con dos candidatas no se elige')
  ...
  assert.match(f, /p\.created_at >= \(op\.created_at at time zone 'UTC'\) - interval '1 day'/)
...
test('🚨 no toca retenciones ni leads del volcado, y solo cierra oportunidades abiertas', () => {
  ...
  assert.match(f, /where id = \$\{f\.id\}::uuid and estado::text in \('competencia', 'en_negociacion', 'pendiente_cliente'\)/)
})
