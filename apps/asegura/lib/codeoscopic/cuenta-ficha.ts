...
export type CuentaFicha = {
  ...
  /** Había al menos una cuenta guardada que no se pudo descifrar. */
  ilegible: boolean
}

export const SIN_CUENTA: CuentaFicha = { iban: null, origen: null, ilegible: false }
...
export async function cuentaDeFicha(
  ...
  try {
    ...
    filas = await prismaAsegura().$queryRaw<{ origen: OrigenCuenta; valor: string | null }[]>`
      with ficha as (
        ...
        where p.correduria_id = ${correduriaId}::uuid
          and p.id = coalesce(
            ${polizaId}::uuid,
            (select p0.id from polizas p0 where p0.correduria_id = ${correduriaId}::uuid and p0.cliente_id = ${clienteId}::uuid limit 1)
          )
      )
      select origen, valor from (
        ...
        select 4, 'recibo_otra_poliza', r.iban, r.fecha_emision
          ...
          join ficha f on f.cliente_id = p2.cliente_id and p2.id <> f.poliza_id
    ...
    // Sin poder mirar no se afirma nada: la ruta lo tratará como «no lo sé» y
    // la pantalla pedirá la cuenta.
    return SIN_CUENTA
  }
  ...
  for (const f of filas) {
    ...
    if (claro === null || claro.startsWith('v1:')) {
      if (f.valor?.startsWith('v1:')) ilegible = true
      continue
    }
    const iban = normalizarIban(claro)
    if (iban && ibanValido(iban)) return { iban, origen: f.origen, ilegible: false }
  }
  return { iban: null, origen: null, ilegible }
}
