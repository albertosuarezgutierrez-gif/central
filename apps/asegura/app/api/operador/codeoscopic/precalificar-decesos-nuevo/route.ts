...
import { precalificarDecesosNueva, type ResueltosDecesosNueva } from '@/lib/codeoscopic/desde-cartera-decesos'
...
import { estadosCiviles, emparejar, lineasDeSeguro, decesosDisponible, type Opcion } from '@/lib/codeoscopic/catalogos'
...
/**
 * `GET /api/operador/codeoscopic/precalificar-decesos-nuevo?clienteId=` — la
 * ficha de DECESOS para un cliente que HOY no tiene ninguna póliza, gratis.
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return error(502, registrarErrorCartera('precalificar-decesos-nuevo/correduria', e), 'No se ha podido resolver la correduría.')
  }
  ...
  try {
    ...
    return error(
      502,
      registrarErrorCartera('precalificar-decesos-nuevo/ficha', e),
  ...
  const decesos = decesosDisponible(lineas)

  const resueltos: ResueltosDecesosNueva = {
    estadoCivilId: estadoCivil?.id ?? null,
    capital: null,
  }
  const pre = precalificarDecesosNueva(origen.cliente, resueltos, hoyIso())
