...
import { prepararRetarificacionNuevaSalud, respuestaRetarificacion, type CuerpoRetarificacion } from '@/lib/retarificar-cartera'
...
/**
 * `POST /api/operador/codeoscopic/salud-nuevo` — presupuesto de SALUD para un
 * cliente que HOY no tiene ninguna póliza (0 en cartera, 03/09/2026). Hermana
 * de `vida-nuevo`: mismas cuatro salvaguardas. **GASTA 0,50€ REALES.**
 *
 * 🚧 **El `risk` de `HealthRisk` NO está verificado contra el fabricante**
 * (ver `lib/codeoscopic/peticion-salud.ts`). El primer intento real puede
 * devolver un 400 que nombre un campo distinto: se lee y se corrige ahí.
 *
 * ── Cuerpo ──────────────────────────────────────────────────────────────────
...
export async function POST(req: Request) {
  ...
  const p = await prepararRetarificacionNuevaSalud({
    clienteId,
    solicitadoPor,
    cuerpo: {
