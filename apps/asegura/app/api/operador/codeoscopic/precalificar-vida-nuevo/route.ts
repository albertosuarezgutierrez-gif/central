...
import { precalificarVidaNueva, type ResueltosVidaNueva } from '@/lib/codeoscopic/desde-cartera-vida'
...
/**
 * `GET /api/operador/codeoscopic/precalificar-vida-nuevo?clienteId=` — la
 * ficha de VIDA para un cliente que HOY no tiene ninguna póliza, gratis, para
 * `apps/plataforma` → `/correduria`. Hermana de `/precalificar-moto-nuevo`,
 * pero sin catálogo de vehículo: no hay nada que navegar, así que la única
 * pregunta al vendor es si el ramo tarifica (`GET /insurance-lines`).
 *
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return error(502, registrarErrorCartera('precalificar-vida-nuevo/correduria', e), 'No se ha podido resolver la correduría.')
  }
  ...
  try {
    ...
    return error(
      502,
      registrarErrorCartera('precalificar-vida-nuevo/ficha', e),
  ...
  const vida = vidaDisponible(lineas)
  ...
  const pre = precalificarVidaNueva(origen.cliente, resueltos, hoyIso())
