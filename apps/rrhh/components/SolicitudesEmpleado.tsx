...
type Resumen = { devengados: number; aprobados: number; en_tramite: number; pendientes: number }
const GRUPOS = tiposPorGrupo()
const COLOR: Record<string, string> = { solicitada: 'text-ink-3', aprobada: 'text-ok', rechazada: 'text-alert' }

function StatVac({ label, valor, color }: { label: string; valor: number; color?: string }) {
  return (
...
export default function SolicitudesEmpleado() {
  ...
  const pctUsado = resumen ? Math.min(100, Math.round(((resumen.aprobados + resumen.en_tramite) / resumen.devengados) * 100)) : 0
      ...
      {resumen && (
        <div className="mb-4">
          <div className="mb-1.5 flex items-center justify-between text-xs text-ink-3">
            ...
            <span>{resumen.aprobados + resumen.en_tramite} / {resumen.devengados} días</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-1 text-center sm:grid-cols-4">
            ...
            <StatVac label="Pendientes" valor={resumen.pendientes} color={resumen.pendientes <= 0 ? 'text-alert' : ''} />
          </div>
        </div>
      )}
