...
const BIENVENIDA = '¡Hola! Soy tu asistente. Pregúntame sobre tus empleados, solicitudes pendientes o cualquier duda de gestión.'
const EJEMPLOS = [
  '¿Qué solicitudes están pendientes de aprobar?',
  '¿Cuántos empleados tengo en activo?',
  '¿Hay vacaciones solicitadas este mes?',
]

export default function AsistentePanelAdmin() {
  ...
  return (
    <section className="mb-4 rounded-[12px] border border-line bg-card p-4">
      <h2 className="text-base">🤖 Asistente</h2>
      <p className="mb-2 text-ink-3 text-sm">Pregúntame sobre empleados y solicitudes</p>

      <div className="flex max-h-[300px] flex-col gap-1.5 overflow-y-auto">
        {mensajes.length === 0 && (
          <div className="max-w-[80%] self-start rounded-[13px] rounded-bl-[4px] bg-paper-2 px-3 py-1.5 text-sm text-ink">
            {BIENVENIDA}
          </div>
        )}
        {mensajes.map((m, i) => (
        ...
        {cargando && (
          <div className="max-w-[80%] self-start rounded-[13px] rounded-bl-[4px] bg-paper-2 px-3 py-1.5 text-sm text-ink-3">
            escribiendo…
          </div>
        )}
        <div ref={finRef} />
