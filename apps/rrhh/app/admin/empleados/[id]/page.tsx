...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const emp = await prisma.$queryRaw<any[]>(Prisma.sql`
    SELECT id, nombre, email, puesto FROM rrhh.empleados WHERE id = ${id}::uuid AND empresa_id = ${empresa_id}::uuid LIMIT 1`)
  if (!emp[0]) notFound()
  ...
  return <ExpedienteClient empleado={JSON.parse(JSON.stringify(emp[0]))} carpetas={carpetas} inicial={JSON.parse(JSON.stringify(documentos))} />
}
