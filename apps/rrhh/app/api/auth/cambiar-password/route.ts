...
export async function POST(req: Request) {
  try {
    ...
    const rows = await prisma.$queryRaw<any[]>(Prisma.sql`SELECT pass_hash FROM rrhh.usuarios_rrhh WHERE id = ${usuario_id}::uuid LIMIT 1`)
    const u = rows[0]
    if (!u || !(await verifyPassword(String(actual ?? ''), u.pass_hash))) {
      return NextResponse.json({ error: 'La contraseña actual no es correcta' }, { status: 400 })
    }
    const pass_hash = await hashPassword(String(nueva))
    await prisma.$executeRaw(Prisma.sql`UPDATE rrhh.usuarios_rrhh SET pass_hash = ${pass_hash} WHERE id = ${usuario_id}::uuid`)
