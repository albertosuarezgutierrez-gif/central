...
import {
  ...
  EntregaEpisPdf, type CamposEntregaEpis, type EpiId,
  InformacionRiesgosPdf, type CamposInformacionRiesgos,
} from '@/lib/plantillas-prl'
import { createElement } from 'react'

export const maxDuration = 60

const TIPOS_VALIDOS = ['autorizacion_maquinaria', 'acuerdo_con_acceso', 'acuerdo_sin_acceso', 'entrega_epis', 'informacion_riesgos'] as const
...
export async function POST(req: NextRequest) {
  ...
  if (tipo === 'entrega_epis') {
    const camposPdf = generarCamposEpis({ empresa, empresa_logo_b64, empleado, campos })
    const faltantes = validarCamposEpis(camposPdf)
    if (faltantes.length > 0) {
      return NextResponse.json({ error: `Faltan campos: ${faltantes.join(', ')}`, faltantes }, { status: 422 })
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const buffer = await renderToBuffer(createElement(EntregaEpisPdf, camposPdf) as any)
    const arrayBuffer = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength) as ArrayBuffer

    const fechaStr = new Date().toISOString().split('T')[0]
    const storagePath = `empleado/${empleado_id}/prevencion_riesgos/entrega-epis-${fechaStr}-${crypto.randomUUID()}.pdf`
    await subirObjeto(storagePath, arrayBuffer, 'application/pdf')

    const nombre = `entrega-epis-${fechaStr}.pdf`
    await prisma.$executeRaw(Prisma.sql`
      DELETE FROM rrhh.documentos
      WHERE empleado_id = ${empleado_id}::uuid AND empresa_id = ${empresa_id}::uuid
        AND carpeta = 'prevencion_riesgos' AND nombre = ${nombre}`)
  ...
  if (tipo === 'informacion_riesgos') {
    const camposPdf = generarCamposRiesgos({ empresa, empresa_logo_b64, empleado, campos })
    const faltantes = validarCamposRiesgos(camposPdf)
    if (faltantes.length > 0) {
      return NextResponse.json({ error: `Faltan campos: ${faltantes.join(', ')}`, faltantes }, { status: 422 })
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const buffer = await renderToBuffer(createElement(InformacionRiesgosPdf, camposPdf) as any)
    const arrayBuffer = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength) as ArrayBuffer

    const fechaStr = new Date().toISOString().split('T')[0]
    const storagePath = `empleado/${empleado_id}/prevencion_riesgos/informacion-riesgos-art18-${fechaStr}-${crypto.randomUUID()}.pdf`
    await subirObjeto(storagePath, arrayBuffer, 'application/pdf')

    const nombre = `informacion-riesgos-art18-${fechaStr}.pdf`
    await prisma.$executeRaw(Prisma.sql`
      DELETE FROM rrhh.documentos
      WHERE empleado_id = ${empleado_id}::uuid AND empresa_id = ${empresa_id}::uuid
        AND carpeta = 'prevencion_riesgos' AND nombre = ${nombre}`)
...
function generarCamposEpis({
  empresa, empresa_logo_b64, empleado, campos,
}: { empresa: any; empresa_logo_b64: string | null; empleado: any; campos: Record<string, any> }): CamposEntregaEpis {
  const hoy = new Date()
  const dd = String(hoy.getDate()).padStart(2, '0')
  const mm = String(hoy.getMonth() + 1).padStart(2, '0')
  const yyyy = hoy.getFullYear()
  ...
  return {
    ...
    obra_centro: campos.obra_centro ?? empleado.centro_trabajo ?? '',
    fecha_emision: `${dd}/${mm}/${yyyy}`,
    epis: (campos.epis ?? []) as EpiId[],
  }
}

function validarCamposEpis(c: CamposEntregaEpis): string[] {
  const faltantes: string[] = []
  if (!c.empleado_nombre) faltantes.push('nombre del empleado')
  if (!c.empleado_dni) faltantes.push('DNI/NIE del empleado')
  if (!c.empleado_puesto) faltantes.push('puesto/oficio')
  if (!c.epis.length) faltantes.push('al menos un EPI entregado')
  return faltantes
}

function generarCamposRiesgos({
  empresa, empresa_logo_b64, empleado, campos,
}: { empresa: any; empresa_logo_b64: string | null; empleado: any; campos: Record<string, any> }): CamposInformacionRiesgos {
  const hoy = new Date()
  const dd = String(hoy.getDate()).padStart(2, '0')
  const mm = String(hoy.getMonth() + 1).padStart(2, '0')
  const yyyy = hoy.getFullYear()
...
function validarCamposRiesgos(c: CamposInformacionRiesgos): string[] {
  const faltantes: string[] = []
  if (!c.empleado_nombre) faltantes.push('nombre del empleado')
  if (!c.empleado_dni) faltantes.push('DNI/NIE del empleado')
  if (!c.empleado_puesto) faltantes.push('puesto/oficio')
