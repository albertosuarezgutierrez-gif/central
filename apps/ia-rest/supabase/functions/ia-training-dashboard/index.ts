...
// ⚠️ RESCATE 12/07: el deploy tenía el PIN de super-admin HARDCODEADO ("9999"),
// que da acceso a un dashboard con datos de TODOS los restaurantes. Movido a
// env/secret para no versionar la credencial. Configúralo como secret:
//   supabase secrets set IA_TRAINING_PIN=... --project-ref <ref>
const SUPER_PIN = Deno.env.get("IA_TRAINING_PIN") || "";

Deno.serve(async (req: Request) => {
  const url = new URL(req.url);
  const pin = url.searchParams.get("pin");
  if (!pin || !SUPER_PIN || pin !== SUPER_PIN) return new Response(authPage(), { headers: { "Content-Type": "text/html; charset=utf-8" } });
