...
serve(async (req) => {
  ...
  // ⚠️ RESCATE 12/07: el deploy tenía este valor HARDCODEADO en el código. Movido a
  // env/secret para no versionar la credencial. Configúralo como secret de la función:
  //   supabase secrets set TG_SEND_SECRET=... --project-ref <ref>
  const secret = req.headers.get('x-secret')
  const expected = Deno.env.get('TG_SEND_SECRET') || ''
