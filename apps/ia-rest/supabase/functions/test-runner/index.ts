...
async function call(slug: string, body: unknown): Promise<{ok:boolean;ms:number;data:unknown;httpStatus:number}> {
...
Deno.serve(async (req: Request) => {
  ...
  // T1 – BRAIN: comanda
  const b1 = await call("brain-parse", { restaurante_id: R_ID, texto: "marchar dos cañas y una ración de croquetas para la mesa uno" });
  ...
  // T2 – BRAIN: cobro
  const b2 = await call("brain-parse", { restaurante_id: R_ID, texto: "cuenta mesa uno tarjeta separada en dos" });
  ...
  // T3 – BRAIN: 86
  const b3 = await call("brain-parse", { restaurante_id: R_ID, texto: "86 las croquetas se acabaron" });
  ...
  // T4 – BRAIN: alergia
  const b4 = await call("brain-parse", { restaurante_id: R_ID, texto: "ojo la mesa uno es alérgica al gluten sin rebozado" });
  ...
  if (brainOk) {
    const c5 = await call("courier-route", { restaurante_id:R_ID, camarero_id:CAM_ID, brain_output:{...d1, mesa_id:MESA_ID, requiere_confirmacion:false} });
  ...
  // T6 – COURIER: 86
  const c6 = await call("courier-route", { restaurante_id:R_ID, camarero_id:CAM_ID, brain_output:{...d3, requiere_confirmacion:false} });
  ...
  const v7 = await call("vox-confirm", { restaurante_id:R_ID, camarero_id:CAM_ID, intent:"comanda", mesa_nombre:"Mesa 1", items:[{producto_nombre:"Cañas",cantidad:2,modificadores:[]}], quip:"marchando, las cañas bien frías como siempre" });
  ...
  const v8 = await call("vox-confirm", { restaurante_id:R_ID, camarero_id:CAM_ID, intent:"86", items:[{producto_nombre:"Croquetas de jamón",cantidad:1,modificadores:[]}] });
  ...
  const m9 = await call("menu-stockout", { restaurante_id:R_ID, accion:"consultar_86" });
  ...
  if (comanda_id_test) {
    ...
    const vf = await call("verifactu-sign", { restaurante_id:R_ID, comanda_id:comanda_id_test, importe_total:12.50 });
    ...
    const vf = await call("verifactu-sign", { restaurante_id:R_ID, comanda_id:newCmd?.id, importe_total:9.80 });
    const dvf = vf.data as any;
