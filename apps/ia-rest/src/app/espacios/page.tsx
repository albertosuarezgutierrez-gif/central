...
export default function EspaciosPage() {
  useEffect(() => {
      ...
      form.addEventListener('submit', async (e: any) => {
        ...
        try {
          ...
          if (r.ok) {
            ...
            try { (window as any).gtag?.('event', 'generate_lead', { method: 'formulario', origen: 'landing-espacios' }) } catch {}
    ...
    document.querySelectorAll<HTMLElement>('[data-ga]').forEach((el: any) => {
      el.addEventListener('click', () => { try { (window as any).gtag?.('event', el.getAttribute('data-ga') || 'cta_click', { origen: 'landing-espacios' }) } catch {} })
    })
...
.btn-wa { display: inline-flex; align-items: center; gap: 8px; background: #25D366; color: #0B3D2E; padding: 13px 24px; border-radius: 9px; font-weight: 700; font-size: 15px; transition: opacity .2s, transform .15s; }
