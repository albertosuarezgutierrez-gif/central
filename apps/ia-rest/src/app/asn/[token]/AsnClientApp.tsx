...
// parseGS1 / detectBarcode viven ahora en @/lib/barcode (reutilizados por el escáner del owner).

// Pantalla de éxito: extraída a su propio componente para que sus hooks
// (useState) no queden tras un return condicional del componente padre.
function AsnOkScreen({ token }: { token: string }) {
...
export default function AsnClientApp({ token }: { token: string }) {
  ...
  if (screen === 'ok') return <AsnOkScreen token={token} />
