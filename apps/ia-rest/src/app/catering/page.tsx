...
export default function CateringPage() {
  useEffect(() => {
    ...
    async function enviarCatering() {
      ...
      try { (window as any).gtag?.('event', 'generate_lead', { method: 'formulario', origen: 'landing-catering' }) } catch {}
    }
    ...
    document.querySelectorAll<HTMLElement>('[data-ga]').forEach((el: any) => {
      el.addEventListener('click', () => { try { (window as any).gtag?.('event', el.getAttribute('data-ga') || 'cta_click', { origen: 'landing-catering' }) } catch {} })
    })
