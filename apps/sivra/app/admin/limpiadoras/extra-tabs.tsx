...
function TabChecklists() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const r = await fetch(`/api/admin/limpiadoras/checklist?property_id=${selProp}`)
  ...
  async function addItem() {
    ...
    setSaving(true)
    await fetch('/api/admin/limpiadoras/checklist',{method:'POST',headers:{'Content-Type':'application/json'},
  ...
  async function saveEdit() {
    if (!editId) return
    setSaving(true)
    await fetch('/api/admin/limpiadoras/checklist',{method:'PUT',headers:{'Content-Type':'application/json'},
  ...
  async function deleteItem(id: string) {
    ...
    await fetch('/api/admin/limpiadoras/checklist',{method:'DELETE',headers:{'Content-Type':'application/json'},
...
function TabInformes() {
  ...
  useEffect(()=>{
    fetch('/api/admin/limpiadoras/usuarios').then(r=>r.json()).then(d=>{
  ...
  async function loadPreview() {
    ...
    const r = await fetch(`/api/admin/limpiadoras/informe?lid=${selLimp}&from=${desde}&to=${hasta}`)
  ...
  function openInforme() {
    if (!selLimp) return
    const desde = firstOfMonth(monthOffset); const hasta = lastOfMonth(monthOffset)
    window.open(`/api/admin/limpiadoras/informe?lid=${selLimp}&from=${desde}&to=${hasta}`,'_blank')
  }
...
function TabFacturacion() {
  ...
  const load = useCallback(async()=>{
    ...
    const [r1,r2,r3] = await Promise.all([
      ...
      fetch('/api/admin/limpiadoras/tarifas'),
      fetch('/api/admin/limpiadoras/facturacion'),
    ])
  ...
  async function generateFactura() {
    ...
    setGenerating(true)
    const r = await fetch('/api/admin/limpiadoras/facturacion',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(gen)})
    const d = await r.json()
  ...
  async function updateEstado(id: string, estado: string) {
    await fetch('/api/admin/limpiadoras/facturacion',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,estado})})
    load()
  }

  async function saveTarifa() {
    if(!newTarifa.limpiadora_id||!newTarifa.importe) return
    await fetch('/api/admin/limpiadoras/tarifas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(newTarifa)})
            ...
            return (
              <div key={f.id} style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:12,padding:16,marginBottom:10}}>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  <button onClick={()=>window.open(`/api/admin/limpiadoras/informe?lid=${f.limpiadora_id}&from=${f.periodo_inicio}&to=${f.periodo_fin}`,'_blank')}
