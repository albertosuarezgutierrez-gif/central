...
export async function POST(req:NextRequest){if(!(await isCronAuthorized(req)))return NextResponse.json({error:"no autorizado"},{status:401});try{const b=await req.json().catch(()=>({}));return NextResponse.json(await runSync(b.days||2,b.maxPages||20,b.from,b.to))}catch(e){return NextResponse.json({error:e instanceof Error?e.message:String(e)},{status:500})}}
