...
// POST /api/pricing/aplicar-propuesta  — Fase 2-B (Paso 4): RAÍLES del agente IA
//
// El agente IA (skill `pricing-agente`) DECIDE precios, pero NO escribe en Smoobu directamente.
// Manda aquí su propuesta y este endpoint la pasa SIEMPRE por los mismos raíles que la IA no
// puede saltarse (lección de los 125€). Orden de la cadena:
//
...
//   6. CIRCUIT-BREAKER                              → si la propuesta CRUDA es disparatada
//      (mueve demasiadas fechas o un % medio enorme respecto al precio actual), ABORTA la
//      pasada entera SIN escribir y devuelve alerta para revisión humana.
//   7. Sólo fechas DISPONIBLES en Smoobu (no tocar reservas).
//   8. Escribe en Smoobu `/rates` (sólo si !dryRun y no abortado).
//   9. AUDITA todo en `pricing_applied` (source='agente') + `pricing_decisiones` (intención
//      del agente, con `motivo` y snapshot de `variables`) — trazabilidad total para el bucle
//      de auto-mejora y para el chat de plataforma ("¿por qué pusiste X el día Y?").
//
// `dryRun` por defecto TRUE: calcula y AUDITA lo que escribiría, sin tocar Smoobu. Sólo escribe
// en vivo con `{"dryRun": false}` (o `?dryRun=false`). Protegido por CRON_SECRET o sesión admin.
