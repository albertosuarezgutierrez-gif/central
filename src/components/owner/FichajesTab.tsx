...
export default function FichajesTab() {
  ...
  const fmtFecha = (d: string) =>
    new Date(d + 'T00:00:00').toLocaleDateString('es', { weekday: 'short', day: 'numeric', month: 'short' })
  ...
  return (
    <div style={{ maxWidth: 780 }}>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontFamily: SM, fontSize: 10, fontWeight: 700, letterSpacing: '.14em', color: C.ink4, textTransform: 'uppercase' }}>Auditoría</div>
        <div style={{ fontFamily: SE, fontSize: 28, fontWeight: 500, color: C.ink, marginTop: 2 }}>Registro de jornada</div>
        <div style={{ fontFamily: SN, fontSize: 13, color: C.ink4, marginTop: 4 }}>
          Control de fichajes · RD-ley 8/2019
        </div>
      </div>

      {/* Trabajando AHORA */}
      {activos.length > 0 && (
        <div style={{ background: `${C.gr}12`, border: `1px solid ${C.gr}33`, borderRadius: 12, padding: '14px 18px', marginBottom: 24 }}>
          <div style={{ fontFamily: SM, fontSize: 10, color: C.gr, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 10 }}>
            Trabajando ahora — {activos.length} persona{activos.length !== 1 ? 's' : ''}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {activos.map(a => {
              ...
              return (
                <div key={a.id} style={{ background: C.bg2, border: `1px solid ${C.gr}44`, borderRadius: 8, padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: SM, fontSize: 10, color: C.ink4 }}>desde {fmtHora(a.entrada_at)} · {dur}</div>
                  </div>
                </div>
              )
            })}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <input type="date" value={desde} onChange={e => setDesde(e.target.value)}
            style={{ fontFamily: SM, fontSize: 12, background: C.bg2, border: `1px solid ${C.rule}`, borderRadius: 6, padding: '6px 8px', color: C.ink, outline: 'none' }} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {!loading && fichajes.length === 0 && (
        <div style={{ textAlign: 'center', padding: 40, background: C.card, borderRadius: 12, border: `1px solid ${C.rule}` }}>
          <div style={{ fontFamily: SE, fontSize: 20, color: C.ink4, fontStyle: 'italic' }}>Sin fichajes en este período</div>
          <div style={{ fontFamily: SN, fontSize: 13, color: C.ink4, marginTop: 8 }}>Los trabajadores ficharán al entrar con su PIN</div>
        </div>
      )}
          ...
          return (
            <div key={fecha} style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                {horasDia > 0 && <div style={{ fontFamily: SM, fontSize: 11, color: C.ink4 }}>{fmtHoras(horasDia)} total día</div>}
              </div>
                ...
                {rows.map((f, i) => (
                  <div key={f.id} style={{ borderTop: i > 0 ? `1px solid ${C.rule}` : 'none', padding: '12px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      {/* Badge rol */}
                      <div style={{ fontFamily: SM, fontSize: 9, fontWeight: 700, color: C.ink4, background: C.bg2, border: `1px solid ${C.rule}`, borderRadius: 4, padding: '2px 5px', minWidth: 34, textAlign: 'center' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {f.notas && editando !== f.id && (
                          <div style={{ fontFamily: SN, fontSize: 11, color: C.amb, marginTop: 2 }}>{f.notas}</div>
                        )}
                      </div>
                      ...
                      <div style={{ textAlign: 'right', minWidth: 90 }}>
                        <div style={{ fontFamily: SM, fontSize: 12, color: C.ink }}>
                          {fmtHora(f.entrada_at)} → {f.salida_at ? fmtHora(f.salida_at) : (
                            <span style={{ color: C.gr }}>activo</span>
                          )}
                        </div>
                        <div style={{ fontFamily: SM, fontSize: 11, color: f.horas_totales ? C.ink3 : C.gr, marginTop: 1 }}>
                    ...
                    {editando === f.id && (
                      <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
                        <input
                          ...
                          placeholder="Nota para Inspección (ej: turno partido, descanso no computable...)"
                          style={{ flex: 1, fontFamily: SN, fontSize: 12, background: C.bg2, border: `1px solid ${C.rule}`, borderRadius: 6, padding: '7px 10px', color: C.ink, outline: 'none' }}
                        />
                        <button onClick={() => guardarNota(f.id)} style={{ padding: '7px 12px', background: C.verm, border: 'none', borderRadius: 6, fontFamily: SN, fontSize: 12, fontWeight: 700, color: '#fff', cursor: 'pointer' }}>Guardar</button>
                      </div>
                    )}
                  </div>
                ))}
