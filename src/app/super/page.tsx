  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: SN }}>
          ...
          <div ref={sistemaRef} style={{ position: 'relative', alignSelf: 'stretch', display: 'flex', alignItems: 'center' }}>
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '14px 16px', height: '100%',
                ...
                color: ['ia_training','sistema','autocuras','soporte','agentes','instagram','blog','proveedores','qa_agent','prospeccion_apify'].includes(tabSuper) ? C.ink : C.ink3,
                borderBottom: `2px solid ${['ia_training','sistema','autocuras','soporte','agentes','instagram','blog','proveedores','qa_agent','prospeccion_apify'].includes(tabSuper) ? C.red : 'transparent'}` }}>
              {'SISTEMA'}
            ...
            {showSistemaMenu && (
              <div style={{ position: 'fixed', right: sistemaPos.right, top: sistemaPos.top, background: C.bg2, border: `1px solid ${C.rule}`, borderRadius: 8, zIndex: 9999, minWidth: 180, boxShadow: '0 8px 24px rgba(0,0,0,.5)', overflow: 'hidden' }}>
                {([
                  ...
                  { id: 'prospeccion_apify', label: 'Apify Sevilla' },
                ] as any[]).map((t: any) => (
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: `clamp(24px, 5vw, 48px) clamp(12px, 4vw, 32px)` }}>
        ...
        ) : tabSuper === 'prospeccion_apify' ? (
          <ProspeccionApifyTab session={session} />
