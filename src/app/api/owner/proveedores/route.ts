...
export async function POST(req: NextRequest) {
  ...
  const supabase = createServerClient()
  const rid = getRestauranteId(req)
  const { nombre, email, telefono, web, contacto_nombre, categoria, notas } = await req.json()
...
export async function PUT(req: NextRequest) {
  ...
  const supabase = createServerClient()
  const rid = getRestauranteId(req)
  const { id, nombre, email, telefono, web, contacto_nombre, categoria, notas, activo } = await req.json()
