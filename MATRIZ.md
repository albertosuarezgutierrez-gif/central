...
| **almacen** | Almacén de eventos/catering (cliente Joaquín Jaén) | `almacen` | ✅ En `apps/almacen`, Root Directory `apps/almacen` (desplegada 15/07/2026, tenant DEMO poblado; tenant real de Joaquín pendiente; Fases 1-4 operativas desde 16/07/2026 — multi-almacén, eventos, empleados+inventario, catálogo público). Compone `@central/module-materiales`. BD compartida. `ignoreCommand` en `vercel.json` ✅ (17/07/2026) y en la matriz de typecheck de `.github/workflows/tests.yml` ✅ (17/07/2026, verificado `tsc --noEmit` 0 errores). ⚠️ Sigue sin `CLAUDE.md` propio ni fila en `docs/FUENTES-DE-VERDAD.md` — ver informe de auditoría. |
