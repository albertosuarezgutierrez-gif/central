...
import { decodificarHtml, norm } from './texto.ts'
...
export function paramsDnploc(direccion?: string | null): ParamsDnploc | null {
  ...
  const puerta = uno(/\b(?:PUERTA|PTA)[:.\s]*([A-ZÑ0-9]{1,4})\b/)
    ...
    // «2º 14» / «3º B»: la forma en que la gente dice su piso (hogar, 02/09/2026).
    // Solo tras un ordinal de planta, para no confundir el número del portal.
    ?? uno(/\b\d{1,2}[º°]\s*([A-ZÑ0-9]{1,3})\b/)
