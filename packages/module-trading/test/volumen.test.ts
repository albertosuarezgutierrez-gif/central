...
test('rvol compara la última sesión con el volumen típico previo', () => {
  assert.equal(rvol([...base, 300], 20), 3)
  assert.equal(rvol([...base, 100], 20), 1)
  assert.equal(rvol([1, 2], 20), null)   // insuficientes
})

test('rvol usa MEDIANA: un spike de earnings no deprime el rvol de los días siguientes', () => {
  // 19 sesiones a 100 + un pico de 1000 (día de noticias); hoy 150.
  const previos = [...Array(19).fill(100), 1000]
  // Con MEDIA la base sería 145 → rvol 1,03 (parecería normal). Con MEDIANA la base es 100 → rvol 1,5.
  assert.equal(rvol([...previos, 150], 20), 1.5)
})
...
test('confirmaVolumen clasifica el respaldo del movimiento (umbral realista 1,5×)', () => {
  assert.equal(confirmaVolumen('alcista', 1.6), 'confirma')   // ≥1,5× = convicción real
  assert.equal(confirmaVolumen('alcista', 1.3), 'normal')     // 1,3× ya NO es "confirma" (era demasiado indulgente)
