...
- **Fase 2 — Eventos/Workflows** (depende de F1)
  ...
  - ✅ **Presupuesto PR 3 (23/09):** aviso al cliente de un presupuesto preparado, con el clic de Alberto: correo desde asegura (sella `enviado_at` solo si el proveedor acepta) o WhatsApp abierto con el texto (`enlace_generado_at`; «Ya lo he mandado» sella el envío). Sin precio ni compañía en el aviso; el token se rota con compare-and-swap. Tarjeta «Presupuestos» en la ficha de póliza. Siguiente: PR 4 (elegir y firmar).
  - ✅ **Presupuesto PR 4 (23/09):** el cliente elige una opción y la firma en el portal (código al correo, `FirmaPropia` sobre el texto exacto, «no es la contratación» en su cara); Telegram a Alberto. Si cambia de compañía (código DGS), firma a la vez la anulación de su póliza a vencimiento (`anulacion.presupuesto_id`), que la cola solo propone cuando Alberto marca «Ya está emitida». Siguiente: PR 5 (datos que faltan para emitir) y PR 6 (carta de mediador).
  - ✅ **Presupuesto PR 5 (23/09):** qué le falta al tomador para EMITIR (correo, DNI, fecha de nacimiento, dirección con número y CP, cuenta), mirado en nuestra ficha + lo propio de sus pólizas. Cuatro estados (ok · falta · en revisión · no legible: un cifrado que no abre no se le pide). El portal lo enseña bajo la comparativa y deja subir el DNI; Alberto lo ve en la tarjeta de presupuestos; el correo solo cuenta cuántos faltan. Siguiente: PR 6 (carta de mediador).
  - ✅ **Presupuesto PR 6 (23/09):** salida B: el cliente que se queda con su compañía firma en el portal la carta que nos nombra corredores de su póliza (código al correo, texto exacto, solo el tomador; no se ofrece si CIMA ya la trae). Alberto la ve en la ficha de la póliza, la manda él y la marca enviada/aceptada/rechazada. Siguiente: la carta por la cola de aprobaciones (como la anulación) y el parte/grúa por WhatsApp de la compañía con el mensaje pre-rellenado.
  - ✅ **Carta de nombramiento por la cola (24/09):** la firmada se propone como `enviar_correo_compania` (`aprobacion.carta_mediador_id`; CHECK: una propuesta a la compañía apunta a anulación O carta), buzón elegido y recordado aparte (`compania_contactos.recibe_nombramientos`), carta adjunta; al salir → `enviada`. La carta lleva ya el **DNI/NIF del tomador** (Alberto: «es necesario»): sin uno válido en la ficha no se ofrece firmarla, y `carta_texto` se guarda cifrado.
  - ✅ **Adjuntos en PDF (24/09):** la carta y la anulación firmadas salen a la compañía en PDF con el justificante de la firma (firmante, método, hora, huella) **más** el `.txt` original, que es el que respalda la huella SHA-256.
  - ✅ **Presupuesto = oportunidad (24/09, #3514):** una sola puerta para abrir oportunidad en la ficha; cada precio REAL guardado se cuelga de la oportunidad abierta del ramo o la abre (`tarificaciones.oportunidad_id`). El formulario se rellena leyendo póliza, recibo o foto (puerto `leer-documento`).
  - ✅ **Pedir datos al cliente por enlace (24/09, #3516):** desde la oportunidad de moto/auto, enlace de un solo uso (14 días) a `/datos/<token>` del portal, que solo pide lo que falta y no enseña datos del cliente. Al completarlo: en negociación, tarea «Tarificar» hoy y Telegram por el feed de actividad. Token en sha256, respuestas cifradas, nada se escribe en la ficha; si la oportunidad ya se cerró, el enlace muere.
...
- **Fase 3 — Financiero** (depende de F2): WF-Comisiones con el tramo del banco (`banco_total`), score de conciliación y descuadres como incidencias. Sin ledger de terceros. ✅ **24/09/2026:** el banco se casa abono a abono (23/09) y los descuadres suben a «Hoy» (`DescuadresComisiones.tsx`, regla pura `descuadresParaHoy`: `descuadra` siempre; `liquidado-sin-cobrar` solo pasados 45 días del fin del periodo; `no-comprobado` deja el contador en «no lo sé»).
- **Fase 4 — Documental** (depende de F1; puede ir en paralelo a F3): Storage + versionado + retención, migración de los `bytea`, WF-Email/Documento. ⏸️ **Aplazado a propósito (medido 24/09/2026):** hay **1 documento (403 kB) y 0 IPID** en `bytea`; `seguros` ocupa 66 MB de una BD de 306 MB. Mover a Storage hoy es construir por construir. Se reabre cuando los ficheros de `documentos`+`ipid` pasen de ~50 MB, y el borrado del `bytea` sigue siendo decisión de Alberto.
...
- **Fase 6 — Automatización avanzada**: fugas de cartera, Next Best Action por cliente (ordenada por reglas, no por LLM), data quality. (WhatsApp Business y telefonía quedan **fuera del roadmap activo**, decisión del 23/09.) ✅ **Next Best Action en la ficha (24/09/2026):** tarjeta «👉 Siguiente acción» arriba del Resumen, `siguienteAccion()` de module-seguros (reglas en orden fijo: recibo devuelto sin cobertura > devuelto en plazo > renovación ≤60 días > presupuesto sin cerrar > póliza declarada que vence > sin canal > coche sin hogar; `sin_comprobar` si faltaba un dato). ✅ **Chip por fila en Clientes (24/09/2026):** la MISMA regla, calculada por lotes para la página del listado (`apps/asegura/lib/acciones-listado.ts`: una consulta por tipo de dato, no una por cliente); `sin_comprobar` se dice con su chip, `null` no pinta nada.
- **Fase 7 — Optimización**:
...
1. **Obligaciones regulatorias como workflows con plazo** (no como agente de compliance). El sistema debería producirlas, no solo guardar datos. Las que conviene modelar [Probable, a validar con la asesoría]:
   ...
   - **Libro registro de operaciones / pólizas intermediadas**. ✅ 24/09/2026: CSV desde el informe anual (puerto `/api/operador/libro-registro`): pólizas de la cartera viva en vigor en el año, sin DNI.
   - **IDD antes de vender**: test de demandas y necesidades, entrega del IPID, información previa del mediador. Todo con evidencia enlazada al presupuesto (PR 4-6 del spec). 🟡 24/09/2026: la aceptación firmada del presupuesto cita ya lo que el cliente tuvo delante (nº de opciones y compañías del presupuesto, información del mediador con su versión de textos y URL). Exigencias y necesidades (24/09/2026): `presupuesto.necesidades` las escribe el corredor en la tarjeta de presupuestos; sin ellas no se avisa al cliente (`sin_necesidades`), el portal las enseña («Lo que me pediste») y la aceptación firmada las cita. IPID (24/09/2026): `seguros.ipid` por compañía+producto (`claveProducto`), se suben en Datos de `/correduria`, el portal los enlaza en cada opción y la aceptación cita la huella de la vigente (o dice que no consta). Sigue fuera el test de demandas y necesidades como cuestionario estructurado.
   - **Formación continua IDD** (horas por persona y año): una tabla y un aviso. ✅ Hecho el 24/09/2026: `seguros.formacion` + puerto `/api/operador/formacion` + bloque en Datos de `/correduria` (15 h/año por defecto, atrasado desde el 1/10). Quien deja de distribuir se anota con fecha (`seguros.formacion_baja`): antes del año sale del resumen; dentro del año, «baja» sin exigir horas.
   - **Prevención de blanqueo**: solo si hay vida-ahorro. Si no, se declara fuera de alcance.
2. **Medir antes de automatizar** (la regla de la casa: «mide el ahorro, no lo supongas»). Antes de la Fase 5 hay que tener una línea base semanal: correos de correduría, documentos, tareas manuales, minutos de Alberto por proceso. Sin ella, «mínimo trabajo administrativo» no se puede verificar y los agentes no se pueden justificar. ✅ **Hecho el 24/09/2026:** «Línea base semanal» en `/correduria` → Datos (8 semanas: correos de correduría, cambios a mano vs del sistema, aprobaciones, documentos, oportunidades; `—` = aún no se medía). Los minutos por proceso no se miden (haría falta cronometrar).
...
5. **Limpieza de tablas legacy sin uso** (`oportunidades`, `gestiones`, `recordatorios`, `ofertas_automaticas`, `mediator_audit_log`, `lead_estado`). Para cada una: decidir reutilizar (`gestiones` → `tarea`) o renombrar a `_legacy_*`. Si no se decide, el siguiente agente o sesión escribirá en la equivocada. ✅ **Decidido el 24/09/2026:** `oportunidades` y `gestiones` se REUTILIZAN (son las de la pieza 1-1); `recordatorios`, `ofertas_automaticas` y `mediator_audit_log` tienen 0 filas y ningún código de central las escribe (los recordatorios del portal son `portal_*`); **no se renombran** porque el CRM de Manuel sigue escribiendo en `seguros` con su esquema Drizzle y un rename podría tumbar la ingesta de CIMA sin error visible. Regla: no escribir en ellas; la auditoría va a `auditoria` y la cola a `aprobacion`. `lead_estado` es un tipo enum, no una tabla.
...
8. **Capacidad de revisión de Alberto**: una sola persona aprueba. La cola de aprobaciones necesita prioridad, caducidad y un tope diario, o se convierte en la nueva bandeja de entrada. 🟡 **24/09/2026:** prioridad (`urgente`) y caducidad (`caduca_at` + retirada automática de lo que ya no es verdad) ya existen. El **tope diario no se pone todavía**: hoy la cola tiene 0 filas y retener una propuesta (p. ej. un recibo devuelto) tiene coste real. Se revisa con la línea base (§N.2, fila «Aprobaciones pedidas»): si pasan de ~20/semana, se pone.
...
7. ❌ **Mensajes con tu corredor — RETIRADO el 26/09/2026** (Alberto: «con WhatsApp el chat no tiene sentido»). Existió del 24 al 26/09 (pestaña del portal + tabla `seguros.portal_mensaje`, 0 mensajes); se borró el código de las cuatro capas y el cliente escribe por WhatsApp desde la barra inferior del portal. La tabla queda vacía en BD porque la toca la función de fusionar clientes.
8. ✅ **(24/09/2026: el portal traduce el TIPO con la tabla oficial de TIREA y explica el estado en lenguaje claro. Desde asegura#852/#853 el CRM guarda la tramitación que manda la compañía —situaciones, acciones, pagos, reserva, total pagado, posición de culpa— en `siniestros.*_cima` (fusionando, claves oficiales de `module-seguros/siniestro-tramitacion.ts`). El cliente la ve en el portal sin reserva, culpa ni nombres (#3537); el corredor, entera en la ficha de siniestros. Reprocesados los 46 SIN de Drive: 100 siniestros con tramitación)** **Siniestros vivos**: el estado que llega de CIMA traducido a lenguaje claro («la compañía ha asignado perito», «pendiente de pago») y lo que se espera del cliente. Menos llamadas preguntando «¿cómo va lo mío?».
