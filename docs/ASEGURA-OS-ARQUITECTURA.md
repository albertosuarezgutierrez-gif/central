...
| 1-3 | Pantalla Vencimientos + modo llamada (según la maqueta aprobada) — ✅ **Vencimientos + seguimiento construidos (23/09)**: `/correduria/vencimientos` (carriles clientes/leads, canal LSSI por lead) y `/correduria/oportunidad/[id]` (estado, perder/aparcar, tareas, historial). ⏳ Falta el modo llamada | `agente-mecanico` / `delegar-codigo` sobre la maqueta | M |
