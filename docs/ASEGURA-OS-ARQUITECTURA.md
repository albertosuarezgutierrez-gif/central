...
8. 🟡 **(24/09/2026: el portal traduce el TIPO con la tabla oficial de TIREA y explica el estado en lenguaje claro con qué hacer y enlace a Mensajes — `explicarSiniestro`/`tipoSiniestroLegible` de `module-seguros-portal`. Falta lo que CIMA no deja pasar: perito, reserva y pagos llegan a 0 de 100, y la ingesta del CRM reduce las «situaciones» y «acciones» del EIAC a cuatro estados; capturarlas pide cambiar `eiac-sin-mapper.ts` del repo `asegura` y las tablas oficiales de claves de situación/acción de TIREA, que no están en el repo)** **Siniestros vivos**: el estado que llega de CIMA traducido a lenguaje claro («la compañía ha asignado perito», «pendiente de pago») y lo que se espera del cliente. Menos llamadas preguntando «¿cómo va lo mío?».
