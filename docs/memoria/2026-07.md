# 🧠 Memoria de sesiones — archivo 2026-07
...
- **💓 El latido de facturas no faltaba: la pasada moría en 504 antes de escribirlo (31/07/2026).**
  ...
  **01/08:** el PR sigue SIN mergear y la pasada de las 06:15 volvió a dar 504 (`agente_latidos` sigue vacía,
  ...
  (NIM timeout, Groq JSON truncado) son los que se comen los 60 s. **PR #1194 mergeado 01/08 07:40** — la
  pasada de las 06:15 de hoy ya corrió con el código viejo (antes del merge); confirmar en la de mañana
  (02/08 06:15) que `agente_latidos` recibe su primera fila con `ok=true`.
...
- **🦐 NUEVA VERTICAL `apps/mariscos` (Mariscos González) — Fase 1 trazabilidad + etiquetado (21/07/2026,
  PR #1055, mergeado el 11/08 tras resolver conflicto).** Origen: reunión con Maricarmen (transcrita en Drive
  `Mariscos gonzales 1_original.txt`). Dolor: recepción a mano y al re-envasar cambia el nº de lote → pierden la
  traza. Entregado: módulo puro `@central/module-pesca` (herencia de lote de origen, `canalLlevaLote`:
  catering/hotel con lote, mostrador sin; 8 tests) + vertical `apps/mariscos` (Next 15, BD compartida, tablas
  `mariscos_partidas`/`mariscos_envasados` — SQL en `apps/mariscos/prisma/sql/`, PENDIENTE ejecutar preview→prod),
  auth `mariscos_session` (env||throw), `vercel.json` con `ignoreCommand`. **Pendiente:** proyecto Vercel
  (Root `apps/mariscos`), ejecutar SQL, sembrar cuenta real, Fase 2 báscula/etiquetadora.
...
- **✅ Importar extractos en CSV (05/07/2026, PR #755, mergeado el 11/08 tras resolver conflicto).** El modal
  "Importar extracto bancario" de `/banca` no aceptaba `.csv`. Nuevo parser puro `apps/plataforma/lib/extracto-csv.ts`
  (+ 6 tests `node --test`): autodetecta separador (`;`/`,`/tab), comillas RFC4180, coma decimal es-ES, fecha ISO o
  `DD/MM/YYYY`, y **agrupa por la columna "Banco"** → una cuenta importada por banco (no funde Kutxabank+BBVA).
  Cableado en `/api/banca/importar` (`origen='csv'`) y UI (`BancaClient.tsx`).
  ⚠️ **CAVEAT sin resolver:** el export de la app NO lleva IBAN → re-importar el export COMPLETO crea cuentas
  `IMPORTADO-<banco>` paralelas y **duplica el ledger** (`dedupe_hash` incluye `cuenta_bancaria_id`, sin guarda
  cross-cuenta para `origen='csv'`). Seguro para un banco/archivo concreto; el backup entero NO.
