...
|---|---|---|---|
| Alpha Vantage | **Conectado.** Cierra el calendario de earnings y las deslistadas; NO cierra H1. | `EARNINGS_CALENDAR` ✅ (ISRG → `reportDate 2026-10-20`, est. 2,63 USD) · `LISTING_STATUS` ✅ (8.491 deslistadas) · `TIME_SERIES_DAILY_ADJUSTED` ❌ *"this is a premium endpoint"* | 21/08/2026 |
...
|---|---|---|---|
| Alpha Vantage | ~25 llamadas/día en el tier gratis. **Sin verificar:** el número sale de un `rate_limit` observado el 21/08, no de la factura. | `trading-analista` (1/día) · `conectores-vigia` (canarios, 1/mes) | `EARNINGS_CALENDAR` se pide UNA vez SIN `symbol` (devuelve el calendario entero) y se filtra en local. Símbolo a símbolo revienta la cuota con una watchlist de 15. |
...
| `trading-analista` | Alpha Vantage | `EARNINGS_CALENDAR` | La guarda `earningsInminente` deja de vetar **en silencio**. Ver `packages/module-trading/src/riesgo.ts` y el estado `desconocido` de `estadoEarnings()`. |
