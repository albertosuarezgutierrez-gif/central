...
| 3 ✅ 23/09 | **El envío** (hecho así: `apps/asegura/lib/envio-presupuesto.ts` + `module-seguros/mensaje-presupuesto.ts`; PATCH `accion:'avisar'|'confirmar_whatsapp'`; tarjeta «Presupuestos» en la ficha de póliza. El WhatsApp abre `wa.me/?text=` SIN número —Alberto elige el chat— en vez de generalizar `canalWhatsapp()`; `destino_hash` sigue NULL: el acceso va por el vínculo de la ficha. Pendiente: cola en Hoy): correo desde asegura + **deep link de WhatsApp reutilizando `canalWhatsapp()`** (solo se añade `mensajePresupuestoWhatsapp()`) + los dos sellos (`enlace_generado_at` ≠ `enviado_at`) + telemetría + **cola en Hoy** con `onContador` | `apps/asegura/lib/correo-presupuesto.ts` · `packages/module-seguros/src/mensaje-presupuesto-whatsapp.ts` · `POST …/presupuesto/{id}/enviar` · `PresupuestosEnCurso.tsx` | ~400-600 (menos que antes: el canal ya existe) |
| 4 ✅ 23/09 | **Elegir y FIRMAR** (hecho: `apps/asegura/lib/presupuesto-aceptacion.ts` + puente `/api/portal/presupuesto`; portal `AceptarOpcion.tsx`. Decisión de Alberto 23/09: si la opción es de OTRA compañía —por código DGS—, el cliente firma en el mismo acto la anulación de su póliza actual a vencimiento, y no sale hacia la compañía hasta que Alberto marca el presupuesto como emitido. El ReRate de reconfirmación NO lo dispara el cliente: lo hace Alberto al emitir, con el flujo de siempre): OTP de firma por el puerto, `seguros.firma`, evidencia, documento firmable, aviso Telegram de «firmado sin emitir» | `POST …/presupuesto/{id}/codigo-firma` y `/firmar` · `@central/core-firma` (+ la decisión de §2.4 sobre dónde vive la orquestación) | ~600-800 |
| 5 ✅ 23/09 | **§4bis** (hecho: `module-seguros/datos-para-emitir.ts` + `apps/asegura/lib/datos-emision.ts`, puente `GET /api/portal/datos-emision`, bloque «Datos para contratar» en el portal y línea en la tarjeta de Alberto; el aviso dice «me faltan N datos tuyos» sin pedirlos. Recortes deliberados: la identidad NO se teclea —se sube el DNI (`POST /api/documento/dni`) y la escribe Alberto con él delante, así que no hace falta cola `conflicto_dato`—; contacto y dirección van por «Mis datos», que ya existía; la cuenta la pone el corredor al emitir porque el mandato SEPA no existe): `huecosParaEmitirDesdeFicha` + `datosDelTomador` + la pantalla de confirmar datos + `origen='portal'` + cola de `conflicto_dato` | `@central/module-seguros` · portal · puerto de escritura | ~700-900 |
| 6 ✅ 23/09 | **Salida B** (hecho: `module-seguros/carta-mediador.ts` + tabla `seguros.carta_mediador` `pendiente → firmada → enviada → aceptada|rechazada`, puente `/api/portal/carta-mediador` + `apps/asegura/lib/carta-mediador.ts`; portal «Nombrarte mi corredor» en la salida B; tarjeta en la ficha de póliza. La carta NO sale sola: la manda Alberto y la marca «enviada»; la cola de aprobaciones para mandarla queda para después) : carta de nombramiento de mediador, firmada con el mismo mecanismo, estados `enviada → aceptada → rechazada` | idea H del banco de ideas | ~500 |
