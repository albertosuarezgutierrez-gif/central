...
- **2026-07-12 · facturas-correo** · hizo: pasada diaria normal — Gmail `newer_than:2d` (2 candidatos, ambos pedidos Amazon de Pilar/familia — Smileat papilla bebé y Sensilis loción, `personal`, no archivados), reenvíos de Pilar (0), buzón manual `_subir_aqui` (vacío) y raíz `FACTURAS 2026` (sin ficheros nuevos desde el 09/07, todos ya cubiertos por avisos de `_DUPLICADOS_BORRAR` existentes — verificados como no-zombis); revisó Petroprix/fal.ai/ASECON (sin correos nuevos de factura) y EMASESA "Bustos 1º DER" (solo duplicados 2025 ya conocidos, sin factura 2026 nueva que conciliar); dudas: **Vía B (Apps Script→Drive) sigue cortada, ya 19 días** (última copia en `_buzon_pdf` sigue en 23/06, `label:PDF-guardado` no trae nada posterior) — recomiendo revisar la autorización OAuth del script cuanto antes, cada día que pasa aumenta el riesgo de perder facturas con PDF; fallos: —; PRs/commits: commit directo a `main` (esta entrada)
