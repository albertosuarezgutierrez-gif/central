...
**01/08:** #1194 se mergeó a las 07:40 (deploy listo 07:44), hora y media DESPUÉS de la pasada de las 06:15,
que por eso volvió a dar 504 con el deploy viejo y dejó `agente_latidos` vacía un día más. Los logs añaden el
porqué del 504: los reintentos de `aiExtractInvoice` (NIM timeout, Groq JSON truncado) son los que se comen
los 60 s. **✅ CERRADO 02/08:** la primera pasada con el arreglo (06:16:57 UTC) escribió su latido —
`ok=true`, «0 factura(s) nueva(s)». El agente late y el vigía deja de saltar; el 0 de hoy ya es un 0 medido,
no un buzón sin mirar.
