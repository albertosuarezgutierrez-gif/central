...
- **✅ Gastos deducibles del N26 metidos en la correduría (03/07/2026, solo BD; N26 no conectado).**
  - Alberto subió el extracto CSV del N26 («Cuenta Principal», 77 movs 2026) y pidió coger los deducibles. Clasificados: **15 movimientos deducibles = −1.345,73 €** (Claude/Anthropic **1.009,98 €** en 9 cargos · Vercel **182,81 €** neto = 183,10 − 0,29 reembolso · OEPM 127,88 tasa de marca · fal.ai 18,23 · Cabify 6,83). El resto (bares, ropa, golf, Amazon, EMP.PUB.GEST.TUR≈golf de Andalucía…) = personal, NO importado.
  - Insertados en `movimientos_bancarios` (cuenta `N26-MANUAL` = cuenta_bancaria `15badaf5…`, cuenta Alberto `4fdc993a…`) con `destino='seguros'`, `destino_confirmado=true`, `requiere_revision=false`, `origen='manual'`, comentario "Importado manual desde extracto N26 (03/07/2026)". El **Cabify −6,83** que ya existía como `personal` se **reclasificó** a seguros (no duplicado) y se **concilió** con su PDF real de Drive (`factura_ref='drive:1Y-RX8wGi5KsaH0gS_cCMK3gsmtOJb3KB'`). N26 sigue **sin PSD2 ni parser CSV** → import manual puntual.
  - **Justificantes:** 11 recibos localizados en el Gmail de Alberto (Anthropic ×5 vía `invoice+statements+acct_1REyrSBNUnCSzfs9@stripe.com` · Vercel ×3 `invoice+statements@vercel.com` · OEPM «Justificante de pago» 13/05 con PDF · fal.ai #TATOGY-00001 · Cabify). Solo el Cabify quedó archivado/enlazado; los demás **sin archivar** (Anthropic/Vercel/fal son HTML sin PDF adjunto; el conector Gmail no baja adjuntos). Los movs salen con ❗ sin justificante (no cambia la deducibilidad).
  - **4 cargos Claude (−163,21 14/04 · −94,39 27/04 · −180 15/05 · −180 14/06) NO están en el Gmail de Alberto** (ni spam/papelera): son de la **cuenta Claude de Manuel Suárez** (hermano, jefe de desarrollo, `info@manuelsuarez.es`, no conectado a la sesión), pagados con la tarjeta N26. Alberto confirma que **la factura está a su nombre** (deducible OK), solo el recibo llega al mail de Manuel → pedirle esos 4 PDF.
  - **⚠️ BLOQUEO memoria: el Apps Script «Facturas a Drive» sigue CAÍDO** — el último PDF copiado a `_buzon_pdf` es del **23/06/2026**; no archiva justificantes nuevos (probable caducidad OAuth del script en el Google de Alberto). Mientras siga parado, la skill `facturas-correo` no puede archivar PDFs automáticamente y hay que subirlos a mano al Drive.
