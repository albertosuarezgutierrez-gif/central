...
- **💶 Agente de pricing sivra — sesión paralela del 27/07/2026: mismo diagnóstico (401 sin
  `CRON_SECRET`, confirmado por `pg_net`) llegado de forma independiente, y el aviso real por Telegram.**
  Esta sesión corrió en paralelo a la que resolvió el bloqueo (ver la entrada "comps de agosto
  refrescados" más abajo, con el detalle completo y el fix #1101/#1102) — sin autorización interactiva de
  Alberto para el rodeo por Supabase, así que no se fabricó nada en `market_rates` (0 comps los 4 pisos).
  Su aportación distinta: **envió el aviso real por Telegram** (`POST /api/internal/alerta`, `ALERTA_TOKEN`,
  `messageId 2362`) con el hallazgo — Luxury Busto y Busto Reform libres a 5 días vista a ~2x el p50 real
  de agosto — que la entrada de abajo confirma independientemente ("sus 214€ en vivo... son del motor").
  El bloqueo real ya está resuelto (endpoints portados a plataforma + `ALERTA_TOKEN` para dry-run). Detalle
  en `docs/AGENTES-BITACORA.md` y en `pricing_aprendizaje` (`ALL/ciclo_27_07_2026_bloqueo_apply`,
  `prop_luxury_busto/pulso_agosto_27_07_2026`).
