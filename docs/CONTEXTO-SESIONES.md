...
- **🆕 Nuevo agente `llm-vigia` — vigía semanal de LLMs gratis (06/07/2026).** A raíz del incidente
  del 405B (ver más abajo), Alberto pidió un estudio semanal automático de si hay una IA gratis que
  convenga meter. Creado como hermano de `github-vigia`: skill `.claude/skills/llm-vigia`, estado vivo
  en `docs/VIGIA-LLM.md`. Tres patas: (1) **watch de deprecación** de los modelos cableados en
  ...
  (3) **mini-eval** de candidatos con 2 prompts fijos. Salida: `docs/VIGIA-LLM.md` + Telegram si merece
  ...
  Rutinas` (prompt `Ejecuta la skill llm-vigia` + `PLATAFORMA_URL`/`CRON_SECRET` al final).
