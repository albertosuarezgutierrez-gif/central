...
- **🧭 ANÁLISIS COMPETITIVO Yurest RR.HH. + diseño "cuadrante desde previsión" — 26/06/2026 (rama `claude/yurest-hr-competitive-analysis-fjodoh`, PR draft #536)**
  Alberto pidió analizar [Yurest RR.HH.](https://www.yurest.com/recursos-humanos/) como competencia de iarest y ver qué añadir. Entregado `docs/competencia-yurest-rrhh.md` (commiteado): mapa de funcionalidades de Yurest vs lo que YA tiene la casa (`apps/ia-rest` + `apps/rrhh`).
  - **Hallazgo:** iarest ya tiene paridad o mejor en casi todo — control horario legal RD 8/2019 (`HorarioTab`), coste de personal con % s/ventas, cuadrante previsto-vs-real (`turnos_previstos`+`compararPrevistoReal` en `@central/module-horario`), fichaje, vacaciones+firma de contratos (iarrhh), y diferenciadores que Yurest NO tiene (cribado de CVs con IA `cv_analisis`, análisis de convenio con IA, RR.HH. pegado al TPV de voz + VeriFactu).
  - **Único gap real:** **cuadrante auto-generado desde la previsión de ventas** (Yurest lo vende como su estrella). Tenemos forecaster (90d+NIM, `/api/owner/forecaster`) y cuadrante manual, pero sin puente.
  - **Diseño en brainstorming (decidido con Alberto, AÚN SIN CÓDIGO):** alcance = dimensionar **y** asignar nombres; disponibilidad = plantilla del dueño − vacaciones/bajas de iarrhh (puente fino por DNI/email, degradación elegante); motor = **determinista puro** en `@central/module-horario` (`dimensionar`+`asignarCuadrante`, testeable) alimentado por la previsión IA (NO LLM para el rostering, por exactitud legal y límite ~60s Vercel). Salida = borradores `turnos_previstos` (nueva col `origen ia|manual`) revisables en `/owner`.
  - **GOTCHA descubierto:** `personal` (iarest) NO tiene `persona_id` ni vínculo con `empleados` (iarrhh, van por `empresa_id`+`persona_id` de core-identity) → el puente de ausencias hay que construirlo (endpoint `/api/operador/ausencias` en iarrhh + matching DNI/email en iarest).
  - **PENDIENTE:** Alberto debe aprobar las 7 secciones del diseño → entonces escribir spec en `docs/superpowers/specs/` y pasar a plan de implementación. Feature #2 (push del turno al empleado) queda fuera de v1.
