...
- **☎️ Teléfonos de siniestros en la hoja de nevera, y el botón que pedía escribir lo ya escrito (05/09/2026).** Dos arreglos pequeños de la correduría. (1) PR #2297: la hoja imprimible pedía el teléfono al que llamar tras un percance y no existía en ningún sitio — `companias_dgs` gana `telefono_siniestros`/`telefono_asistencia`, y como desde este contenedor la política de red bloquea las webs de las compañías (mapfre.es, allianz.es, occident.com, reale.es) y los comparadores se contradicen entre sí (para Mapfre: tres números distintos según la fuente), los campos se quedan a NULL **a propósito** en vez de rellenarse con un dato sin verificar. (2) PR #2301: la lista de renovaciones era la única de `/correduria` sin los iconos de llamar/WhatsApp/escribir — medido antes de tocar nada, 17 pólizas de 15 fichas en 90 días, con 60% teléfono y 53% email (proporción muy por encima del 16% del buscador general, porque quien renueva es cliente vivo). (3) PR #2366: tras terminar el backfill del DNI, el botón seguía diciendo «Escribir 476 índices» encima de «✅ Escritos 476» porque la cifra del botón venía del plan calculado al cargar la página, no de lo ya escrito — ahora manda `restantes` y el botón desaparece al llegar a 0.
