...
- **🩹 BIDV004566 (Occident, ramo 211 «accidentes») sin coberturas ni prima pese a estar confirmada por CIMA — mapper nunca reconocía `RiesgoAccidentes` (13/09/2026).** El enum `seguros.tipo_seguro` ya tenía `'accidentes'` desde el 09/09 (migración en `central`), pero `classifyTipoSeguro()` del CRM (`asegura`) nunca se actualizó → `tipo_seguro_no_clasificable` en cuarentena. Confirmado contra el XML EIAC real (Alberto, acceso directo a `portal.cimaseg.es`). PR `asegura#822` mergeado (mismo patrón que LOO-807/`RiesgoComercios`) + sync de 8 sitios que TS obligaba por exhaustividad del enum. En paralelo otra sesión arregló un bug hermano (`asegura#821`, `classifyTipoSeguro` solo miraba `riesgos[0]`) y añadió `?batchSize=` al reconcile (`asegura#823/#824`) — sin roce, mergeados los tres. Reconcile de prueba tras deploy (`batchSize=50`) **verificado en BD**: BIDV004566 ya tiene `tipo=accidentes`, `prima_bruta=85.57`, 9 coberturas. El `npm audit` crítico también cerrado: PR `asegura#827` (bump aislado de `next` 16.2.3→16.3.5, **no** el Dependabot #820 que agrupa 23 paquetes y tiene su propio CI roto por `libphonenumber-js`) — 0 críticos, mergeado. 🚨 **Ojo de método: mi primer recuento de la cuarentena (19 SIN/REC «ajenos a la cartera») era FALSO** — otra sesión seguía lanzando reconciles en paralelo mientras diagnosticaba, y 13 de esos 19 eran tuyos y ya se habían enganchado al comprobarlo con calma (normalizando el número). Alberto tenía razón al cuestionarlo («si viene de CIMA, ¿no es mi cartera?»): sí lo es, «sin_poliza_en_cartera» solo dice que no hay fila con ESE número exacto, no que no sea tuya. Recontado en caliente: 0 POL, y de los SIN/REC reales solo quedan **3 antiguos** (Allianz C0109, desde el 24/06, sin match ni normalizando) y **3 recibos de Occident de Grupo ELCA 83** (`548733213`/`548733639`/`548740087`, ya cobrados, 256-388€) que **Alberto no encuentra ni buscándolos directamente en CIMA** — hipótesis suya: Plus Ultra sin volcar (encaja con el patrón ya conocido de Plus Ultra→C0468). Un cuarto caso (recibo de `549147797`, Instituto Studium, póliza que Alberto acababa de dar de alta a mano) se resolvió solo con un reconcile más una vez creada la póliza — no era bug, solo timing. **Sin tocar el emparejador en ningún caso** por el riesgo de colgar un dato de la póliza equivocada. Pendiente: respuesta de Occident sobre las 3 de Grupo ELCA.
- **🔎 Buscador de la cartera (asegura) no encontraba «Alberto Suarez» sin tilde, y una columna migrada nunca se aplicó (13/09/2026).** `porNombre`/`porCiudad` de `lib/cartera-busqueda.ts` usaban `contains/insensitive` de Prisma (ignora mayúsculas, NO acentos); reescritos a SQL crudo con `unaccent()` en los dos lados (con reintento sin la extensión, patrón de `porRiesgo`). Aparte, guardar la dirección de un cliente daba 500: `seguros.clientes.contacto_confirmado_at` estaba en el schema Prisma y en un SQL de migración (`2026-09-08_clientes_contacto_confirmado.sql`, «NO se aplica desde el repo») pero nunca se ejecutó contra la BD real — aplicada ahora vía Supabase MCP y verificada.
