...
- **🤖 agentes-entrenador: la poda de la bitácora llevaba DOS pasadas fallando en silencio (06/09/2026).** Las podas del 23/08 y 30/08 se declararon "hechas" en su propio texto, pero las 42 entradas de ese rango seguían todas en `docs/AGENTES-BITACORA.md` — podadas ahora de verdad, y añadido guardarraíl a la skill para releer el archivo antes de cerrar la pasada. Diagnóstico del resto: todo sano salvo `mercado-booking`, cuya pasada acotada "PRIORIDAD TEMPORAL" lleva **7 días seguidos** repitiéndose sobre un objetivo ya cumplido — vive en el prompt de un trigger externo que ninguna sesión puede editar; escalado por Telegram para que Alberto lo quite a mano. PR draft: rama `claude/upbeat-shannon-rdw66d`.
