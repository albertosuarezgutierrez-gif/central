...
- **🔐 PANEL DE SECRETOS · FASE 2 (escritura blindada) — 24/06/2026 — mergeado #502 (`22d8aae`) + #494 (cierre memoria)**
  ...
  - **Archivos:** `lib/vercel-env.ts` (`upsertProjectEnv`, write-only), `app/api/operador/secretos/set/route.ts` (POST con los 6 candados), `app/(usuario)/operador/secretos/SecretosClient.tsx` (botón ✏️ Editar + form inline valor+contraseña), `lib/secrets-registry.ts` (campos `editable`/`vercelProject`), `prisma/sql/2026-06-24_secrets_audit.sql` (tabla, **ya aplicada** vía Supabase MCP).
  - **Activación (hecho por Alberto):** token Vercel `plataforma-secrets-panel` (scope team, Never) puesto como `VERCEL_ADMIN_TOKEN` (Sensitive) en *plataforma* Production+Preview. ⚠️ Token con escritura sobre los 5 proyectos del team → rotarlo periódicamente.
  - **Claves editables:** GEMINI/GROQ (plataforma) · RESEND_WEBHOOK_SECRET/GOOGLE_PLACES_API_KEY/TURNSTILE_SECRET_KEY (ialimp) · GOOGLE_CLIENT_SECRET/CLOUDINARY_API_SECRET/GH_PAT (ia-rest) · SERPER_API_KEY (sivra) · **CIMA_WSE_PASSWORD (plataforma — alta nueva, correduría/seguros Codeoscopic; rama `claude/secrets-register-cima-wse`).**
  ...
  - **Caveat de prueba:** no se pudo verificar la escritura real end-to-end desde el contenedor efímero (proxy bloquea `*.vercel.app` + preview tras SSO de Vercel). Verificado por build (la route compiló en el bundle) y revisión de código; la prueba viva (login+2FA) la hace Alberto en producción.
