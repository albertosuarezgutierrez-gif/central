...
- **✅ icono deducibilidad IRPF en TODOS los gastos (02/07/2026, PRs #655 + #657 mergeados).**
  - ✅ = deducible; ❌ = no deducible. Sin icono en ingresos ni traspasos (`bucket=traspaso`).
  - **Dashboard** (`dashboard/page.tsx`): función pura `iconoDeducible(destino, importe)`. Aparece en `MovRow` (Saldo por cuenta) y `TopGastosWidget` (Mayores gastos). PR #655.
  - **GastosTab** (`finanzas/GastosTab.tsx`): usa campo `m.deducible: boolean` (ya calculado en servidor). Aparece en componente `Fila` (gasto individual) y `Grupo` (agrupación por comercio), junto al importe. PR #657. Sin cambios de BD ni API.
