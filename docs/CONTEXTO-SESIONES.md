...
- **🟡 facturas-correo (09/07/2026, PR #796 — quedó 20 días en draft, mergeado recién el 29/07).**
  Pasada diaria: 3 recibos Stripe/Anthropic Ireland (38,25€ c/u, uso de API Claude, `seguros`)
  archivados en Drive, pendientes de que entrase el cargo bancario. Revisó el backlog manual de
  `FACTURAS Apartamentos/2026` en Drive (25 ficheros sueltos, Paso 1-bis) — la mayoría resultó ser
  duplicados de facturas EMASESA/Endesa/Dimitri ya archivadas y conciliadas en pasadas anteriores.
  Detectó el Apps Script "Facturas a Drive" (vía B) parado desde el 23/06 (16 días en ese momento) y
  dejó 6 casos "Para tu decisión" — **nota al leer esto ahora:** dado que han pasado 20 días y hubo
  pasadas de `facturas-correo` los días 11/07, 12/07 y 24/07 (ver `docs/AGENTES-BITACORA.md`), lo más
  probable es que esas 6 dudas y el corte del Apps Script ya estén resueltos o superados — esta
  entrada queda solo como registro histórico, no como pendiente activo. **Lección operativa:** un PR
  de solo memoria puede quedar mucho tiempo en draft sin revisión — para pasadas que no tocan código,
  valorar commitear directo a `main` en vez de abrir PR (como ya hacen otros agentes programados).
