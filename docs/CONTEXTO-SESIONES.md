...
- **✅ video IA Instagram — FUNCIONANDO en producción, flujo ASÍNCRONO (02/07/2026, PRs #656, #658 y #661 mergeados).**
  ...
  - **Flujo ASÍNCRONO final (PR #661)**: el flujo síncrono también moría (el 110s de AbortSignal saltaba antes que fal.ai). EF `ig-video-gen` **v3** con 2 acciones: `action=start` (encola en fal.ai, devuelve `requestId` al instante) y `action=status` (consulta la cola; `COMPLETED` ⇒ `videoUrl`). El endpoint: `GET /api/ig-ai-video?tipo=X` → `{ok, jobId, consultar}` en ~2s; `GET /api/ig-ai-video?jobId=<id>` → `{estado}` o `{estado:'COMPLETED', videoUrl}`. Para i2v pasar además `&modelo=fal-ai/wan-i2v`.
