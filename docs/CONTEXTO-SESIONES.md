...
**(24/09/2026)** — Idealista: de alertas de correo a CONECTOR. Alberto quita las alertas de Idealista (Fotocasa sigue por correo). Nueva rutina `idealista-radar`: casas de 3+ dormitorios cerca de la playa en 9 núcleos de la costa de Huelva → `POST /api/subastas/mercado/idealista` → mismo `upsertComparable` (los avisos siguen saliendo del cron `subastas-mercado`). Mapeo puro `comparablesDesdeMcpIdealista`: **la zona la decide el núcleo buscado** (Idealista titula Matalascañas como «…, Almonte» y la lente 🌊 no la reconocía) y lo que cae fuera del radio se descarta. Latido `subastas_idealista` (30 h). Centros nuevos: El Portil, Isla Canela. El conector NO sirve desde un cron: solo sesión/rutina. **Avisos ⬇️ solo de BUENAS bajadas** (`esBuenaBajada`: última ≥5% o ≥10% acumulado con 2+), 🌊 primero; y la bajada que declara el portal (`priceDropInfo.formerPrice`) entra en `upsertComparable` como 1ª bajada — si no, un anuncio que entra ya rebajado no avisaba nunca.

**(24/09/2026)** — Rediseño de la ficha, fase 1: oportunidades en la ficha del cliente (tarjeta «💼 Oportunidades» en Resumen). Se abren a mano con su PRIMER PASO obligatorio (tipo + fecha; si ya hay una abierta del ramo, 409 y se enlaza esa), se corrigen ramo/vencimiento/compañía/prima (solo abiertas; historial con antes/después, la compañía solo «cambió»), se ganan eligiendo la póliza, y se DESCARTAN (motivo nuevo `error_alta`: no se borra ni cuenta como venta perdida; CHECK de BD actualizado). Decisiones por defecto de las preguntas del estudio: descartar en vez de borrar; «perdida» sigue exigiendo motivo en Seguimiento.

**(24/09/2026)** — /correduria más rápida (Alberto: «la carga es súper lenta»). Se montaban las 8 pestañas al entrar (~34 peticiones, ~28 al puerto de asegura, que atiende 5 a la vez) para enseñar solo Hoy. Ahora cada pestaña se monta la primera vez que se abre (Recaptación y Blog siguen montadas: alimentan «esperan tu OK»); la matriz de Comisiones se pide al abrirla; «Clientes en el portal · 24 h» de Hoy va plegado y se lee al abrirlo. La ficha del cliente pinta un esqueleto (`loading.tsx`) al cambiar de pestaña. Coste: el badge de una pestaña no abierta no se pinta hasta abrirla. Estudio de rediseño: https://claude.ai/code/artifact/1c1514f7-02e4-4c33-a8b7-4e6091352c49
...
- 🚨 PENDIENTE (decisión de Alberto, toca precios): el MOTOR usa esa misma recta mezclada → en Luxury infravalora las fechas con antelación ~14 % (G=100€/n → base 69 en vez de 83). Y averiguar qué pone el +10 % de última hora en los Busto (Smoobu o extranet).
...
- **📡 Manuel contestó: el webhook de Codeoscopic EXISTE pero apunta a su CRM y descarta el payload real (13/09/2026).**
  ...
  **array de 2 elementos `{insurance}` cada ~30 min** (1.671 rechazos en `operational_events` desde el 25/06, antes de emitir
  ...
  `PUBLIC` del middleware (Resend también recibía el login). PR #2901 mergeado y verificado en prod (`GET` → 405 con
  `x-matched-path`); envs `CODEOSCOPIC_WEBHOOK_BASIC_USER/BASIC_PASS` puestas por Alberto en `central-asegura`. **Decisión de
  Alberto:** primero emitir; el repunte de la URL a JM se pide cuando la emisión funcione al 100 % y **con dominio propio,
