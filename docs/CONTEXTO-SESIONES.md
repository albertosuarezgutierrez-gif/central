...
- **🌀 DOMÓTICA TUYA — ventilador de techo de Socorro/House Sevillana (03/07/2026, PR #714 draft, pendiente de envs).**
  - Alberto tiene un ventilador de techo **CREATE con WiFi** (hardware Tuya) en C/ Socorro 24. Quiere control desde plataforma + programaciones. **Regla exacta acordada:** día de llegada del huésped a las **15:00 Madrid**, si en Sevilla hace **>30 °C** (Open-Meteo, sin API key) → encender **SOLO el ventilador (la luz nunca)**; día de check-out a las **11:30** → mandar **apagar siempre** (apagar apagado es inocuo; cubre el desfase del mando RF, que no sincroniza estado con el cloud).
  - **El ventilador se re-emparejó de CREATE Home a Smart Life** (03/07, Alberto in situ; modo emparejamiento del Wind Calm: mando, 1H+4H ~5 s hasta pitido). El QR de vinculación se escanea desde Smart Life.
  - **Construido en plataforma** (spec `docs/superpowers/specs/2026-07-03-domotica-tuya-ventilador-design.md`, plan en `docs/superpowers/plans/`): `lib/domotica/{tuya,meteo,programador}.ts` (cliente OpenAPI con firma HMAC v2 sin SDK npm + mapeo DP dinámico; Open-Meteo fail-safe; lógica pura de ventanas con 15 tests `node --test` incl. DST y salida+entrada mismo día), tablas `domotica_dispositivos`/`domotica_log` (aplicadas en Supabase; índice único de idempotencia por reserva; REVOKE anon/authenticated), rutas `/api/sivra/domotica/{dispositivos,dispositivos/[id],descubrir,comando,programador}`, cron `25,55 8-15 * * *` (decide en hora Madrid), UI `/sivra/domotica` (sidebar «🌀 Domótica», inline styles con tokens — plataforma NO usa Tailwind), guía `docs/DOMOTICA-TUYA.md`.
  - **PENDIENTE (Alberto, ~10 min desde ordenador):** cuenta en **platform.tuya.com** (NO tuya.com, y NO desde móvil) → Cloud Project data center **Central Europe** + trial IoT Core → Link App Account por QR con Smart Life → `TUYA_CLIENT_ID`/`TUYA_CLIENT_SECRET` en Vercel plataforma → en `/sivra/domotica` «Buscar dispositivos» y vincular el apartamento de Smoobu. Después: prueba real de encender/apagar. **El trial de IoT Core caduca cada ~6 meses** (renovación gratis; el error de la API lo dice en el mensaje).
...
- **🔥 CRM ia-rest: botón "✅ Enviar email" MUERTO + paso a envío automático (03/07/2026, PR #709 MERGEADO, en producción; primera tanda auto de 10 catering enviada a las 09:29).**
  ...
  - Sin tocar: WhatsApp/Instagram siguen manuales; `crm-envio-auto` (doble cerrojo `ENVIO_AUTO_ACTIVO`) queda como estaba para el flujo `email_draft` del pipeline.
  - **2ª iteración (misma sesión) — MENSAJE TIPO GENÉRICO + restaurantes/bares nacional (elegido por Alberto con AskUserQuestion):**
    - `construirEmail` ya NO tiene 4 plantillas por vertical: **UN email genérico** (opción A elegida: gancho margen «¿cuánto margen real te queda? 🍽️» + abanico de servicios voz/almacén/contabilidad/RRHH/eventos/CRM + CTA a la web). **NO cambiar el texto sin OK de Alberto.** Los CFG apuntan todos a la home (`www.iarest.es`) con utm por vertical; la home monta `LandingClickTracker` → el aviso Telegram «👀 X ha entrado en la web» (ya existía en `/api/leads/track-click`) sigue funcionando.
    - **Volumen elegido: ~40/día laborables** (Resend gratis = 100/día, 3.000/mes): catering 10→**15**, nuevo cron **`crm-emails-restaurantes`** (7:45, **15**/día, vertical `restaurante` añadido a `proponerEmailsVertical`), Sevilla 12. Apify: +6 queries nacionales de restaurantes/bares (Madrid, Barcelona, Valencia, Málaga, Bilbao, Zaragoza); el aviso de ingesta deriva el ámbito de la query (antes todo restaurante decía «Sevilla»).

- **✅ Bandeja «Por revisar» de /finanzas/gastos: pasada IA completa 235 → 15 (03/07/2026, solo datos + reglas; sin código).**
  - Alberto pidió revisar la bandeja con agente/IA («duda pregúntame»). Clasificados y CONFIRMADOS en `movimientos_bancarios` (BD compartida, cuenta Alberto `4fdc993a…`): **203 → `personal`** (compras tarjeta: súper/bares/farmacias/colegio/Amazon…, incl. 4 recibos TE-TotalEnergies con **CUPS ES0031102227887014EY0F = Monte Carmelo** anotado en `comentario`), **14 → `turistico_pisos`** (Smoobu 1.018 €, Sique Brilla ×3, Lavandería Girandillo ×2, DIGI ×5, Netflix, Amazon 47,08, IKEA 888,89), **1 → `turistico_pisos`** (CREATE 123,45 — factura ya archivada en Drive `FACTURAS Apartamentos/2026`), **2 → `turistico_duplex` + `propiedad_id=prop_duplex_center`** (Finetwork).
  - **Reglas** (`banca_destino_reglas`): + `GALOS CMI→personal`, `SMOOBU/SI QUE BRILLA/LAVANDERIA EL GIRANDILLO/DIGI SPAIN TELECO→turistico_pisos`, `RECIBO CIRCULO MERCAN→personal (deduccion deportiva_and)`. **ELIMINADA la regla `TE ELECTRICIDAD Y GAS ESPANA→turistico_duplex`** (mina: los recibos TE de Kutxa son el gas de Monte Carmelo=personal; el Dúplex ya está en Endesa/BBVA).
  - **⏳ 15 DUDAS pendientes de Alberto** (siguen en la bandeja; el AskUserQuestion no llegó a entregarse): 5× luz TotalEnergies ene–feb SIN CUPS en concepto (−310,01/−117,67/−116,15/−53,41/−4,14 — los PDF están en Gmail «Facturación Total Gas y Electricidad España», habría que abrirlos para casar CUPS↔importe; Total suministraba Monte Carmelo + Socorro + Bustos, todo al Kutxa ****0855) · 3× RECIBO AYTO. SEVILLA (−282,07 17/02, −282,07 24/02, −130,93 16/04 — ¿IBI de qué inmueble?) · TRANSF. 2100 (−1.093,84 y −798,60, marcadas pisos) y TRANSF. 0182 (−350,09 «abono factura revisión») · HomeExchange (−163,37 ¿personal o pisos?) · D CULTO (−150,60 pre-marcado pisos) · TaskRabbit «TASKER ON TAS» (−85,41 — ¿montaje del IKEA 888,89 de un piso?).
  - Pendiente menor: pasar `POST /api/finanzas/gastos/revisar-cuota-batch` para sincronizar `deduccion_cuota_tipo` (guardería Escuela Infantil, deportiva Círculo) con `fiscal_perfil`.
...
- **✅ Home `/dashboard` reducida a RESUMEN de verdad (02/07/2026, PR de esta sesión, 5ª iteración del día).**
  ...
  - **Fuera (todo tiene página dedicada):** strip Hoy, widget Correduría (→ `/correduria`), widget Apartamentos (→ `/apartamentos`), Pendiente de cobrar OTA, Top gastos del mes, gráfica Cobros de pisos 6 meses (`CobrosPisosChart.tsx` BORRADO), Reservas por piso ±7d (→ `/sivra/calendario`), Comparativa mes vs anterior, evolución mensual (`EvolucionChart.tsx` BORRADO), Gastos por categoría (→ `/finanzas/gastos`). El Promise.all de datos pasa de 16 fetches a 5. ~600 líneas menos; página de 1021 → ~440.
  - Enlaces del AlertasBanner actualizados a `/finanzas/gastos` (antes `?tab=gastos`). Las funciones de `lib/banca.ts` que quedaron sin consumidor (`getSerieCobrosPisos`, `getCobradoPisos`, `getTopGastosMes`, `getEvolucionMensual`, `getComparativaMensual`, `getGastosPorCategoria`, `getStripHoy` inline borrada) se dejan en la lib a la espera de la decisión de Fase 2 (punto 3 de la lista de duplicidades).
- **✅ AUDITORÍA DE DUPLICIDADES en toda el área de usuario de plataforma + Fase 1 de limpieza (02/07/2026, PR de esta sesión, 4ª iteración del día).**
  - Petición de Alberto tras la limpieza de `/finanzas/fiscal` (PR #695): «revisa el resto de páginas, hay mucha duplicidad y eso puede traer errores». Dos agentes barrieron TODAS las páginas de `app/(usuario)/`. **Mapa completo en los hallazgos de abajo; Fase 1 aplicada:**
  - **`/finanzas` (FinanzasClient) desmantelada a lo no-duplicado:** sus tabs Gastos y Fiscal eran copias 1:1 de `/finanzas/gastos` y `/finanzas/fiscal` (con `TramoBar`, `SituacionFamiliarForm`, Modelo 179 y tabla trimestral COPIADOS byte a byte — por eso el fix de hoy solo llegó a una copia). Quedan solo tabs **Ingresos** y **Categorías** (contenido único); `?tab=gastos|fiscal` redirigen a las páginas nuevas; KPI «Base imponible est.» de cabecera eliminado (vive en fiscal); ~500 líneas de código duplicado borradas.
  - **Fix landmine `v_movimientos_activos` en dashboard:** `getStripHoy` y `getGastosSinClasificar` (`dashboard/page.tsx`) leían `movimientos_bancarios` SIN filtro `duplicado_estado` → podían contar duplicados ignorados. Pasados a la vista canónica. (`lib/banca.ts` ya filtraba manualmente en todas sus queries — no tocado.)
  - **Enlaces a la página huérfana reapuntados:** `/finanzas/tarjeta-credito` → `/finanzas/gastos` (2 sitios); `/correduria` → `/finanzas/fiscal`.
  - **Etiqueta engañosa corregida:** `/sivra/income` rotulaba «Ingresos brutos» un total de `incomes.amount`, que es NETO de comisión OTA en todo el sistema → «Ingresos netos».
  - **⏳ FASE 2 — duplicidades detectadas PENDIENTES de decisión de Alberto** (alto riesgo de números que no cuadran entre páginas):
    1. **Ingresos por piso agrupados por `incomes.date` vs `incomes.checkIn`**: `/apartamentos` (lib/propiedades.ts, por `date`) vs dashboard/`resultado-pisos`/`sivra/fiscal` (por `checkIn`) → el mismo piso/año da totales distintos según la página. Hay que elegir UN eje (checkIn es el criterio fiscal).
    2. **4 definiciones de «gasto del piso»**: sivra/fiscal (filtro `revisado`), apartamentos (sin reparto), apartamentos/[id] (con `movimiento_reparto`), resultado-pisos (reparto + tarjeta + Giraldillo). Unificar en una función lib.
    3. **`getCobradoPisos` vs `getSerieCobrosPisos`** (dashboard): misma cifra «cobrado pisos mes» por dos code-paths (tabla vs vista); hoy coinciden, pero divergirán al primer cambio. Unificar.
    4. **`/finanzas/proyeccion` vs card «🔮 Fin de año» de fiscal**: misma proyección (`getProyeccionFiscal`) en dos páginas; proyección además reimplementa los tramos en cliente (`TRAMOS_IRPF` local). Decidir si la página proyección se queda (simulador + patrones) o se fusiona en fiscal.
    5. **AlertasBanner del dashboard**: el CONTADOR «por revisar» (getAlertas, filtra duplicados) y el IMPORTE contiguo (getGastosSinClasificar, criterio `requiere_revision`) usan criterios distintos → pueden contradecirse en la misma línea.
- **✅ SEMANA TEMÁTICA de contenidos + formato CARRUSEL + look nuevo de Reels (02/07/2026, PRs #692, #696 y #699 mergeados).**
  - **Plan semanal (decisión de Alberto):** domingo el briefing propone 3 ideas de blog por Telegram → elige una → **toda la semana desarrolla ese tema**: lunes 🗂️ carrusel «las 3 claves», miércoles 🎬 Reel IA, viernes 🗂️ carrusel «los 3 errores». Crons en `apps/ia-rest/vercel.json`: briefing `30 8 * * 0`, instagram `0 8 * * 1,3,5`.
  - **PR #699:** `briefing-callback` ya NO siembra 3 posts de imagen programados — guarda `instagram_semana` `en_curso` y el cron `instagram` genera cada pieza fresca en su día (`temaSemanal()` manda sobre `elegirTemaConContexto`; fallback libre si no hubo elección). `coletillaBlog()` añade el link al blog en captions cuando está `publicado` (publicación del blog sigue manual en /super). Botón «🔄 Otras ideas» del briefing ARREGLADO (estaba muerto por early-return): relanza el cron con `?manual=1`.
  - **PR #696 — CARRUSEL:** `?formato=carrusel` (y ahora lun/vie por calendario): NIM genera gancho+3 claves+caption; 5 diapositivas con el motor `ig-img` (portada `pregunta` «Desliza →», claves `cita` numeradas, cierre de marca con slogan) guardadas en `video_job.slides`; Telegram `ig_aprobar_carrusel` → `publicarCarrusel()` (Graph API, existía sin uso). Primer carrusel de prueba generado («¿Ineficiencia?», pendiente de OK de Alberto).
  - **PR #692 — look Reels:** la pastilla roja no convenció → watermark «ia.rest» sutil (blanco 75%, sin caja) + **cierre de marca de ~2s** (tarjeta `ig-img` con el slogan, subida una vez a Cloudinary como `iarest_endcard_v1` y empalmada con `fl_splice`; **`l_fetch` está RESTRINGIDO en esta cuenta Cloudinary** — los assets deben subirse). La URL con cierre se valida con HEAD antes de usarse (si el splice falla → versión sin cierre). Pendiente: Alberto quiere el producto/logo DENTRO de la escena → i2v con capturas reales de la app (siguiente tanda; pedirle 2-3 pantallazos de /edge o KDS).
  - **Añadido después (misma sesión, PR de A/B+testimonios):** el carrusel genera **2 portadas (ganchoA/ganchoB)** y Alberto elige cuál publicar desde Telegram (`ig_aprobar_carrusel:{id}:a|b`; la elección queda en `video_job.gancho_elegido` para el futuro bucle de aprendizaje). El carrusel del **viernes alterna por semana**: errores ↔ «frases de barra» (citas INVENTADAS pero realistas con atribución GENÉRICA tipo "Dueño de bar" — **regla: NUNCA presentarlas como testimonios de clientes reales**).
  - **Ideas PENDIENTES para siguientes tandas** (lista viva, por impacto): (1) 💬 **DM automático con lead magnet** («comenta CARTA y te llega la guía» → DM + alta en CRM; Graph API lo soporta) — la de más conversión; (2) 📊 informe semanal de métricas → selector de temas (bucle de aprendizaje; ya se guarda `gancho_elegido` del A/B); (3) 📱 multi-plataforma: mismo Reel a TikTok/YouTube Shorts, carrusel a LinkedIn (APIs de subida, mismo OK Telegram); (4) 🎬 Reels i2v con capturas reales de la app (pedir a Alberto 2-3 pantallazos /edge o KDS); (5) 📧 newsletter mensual con los blogs del mes a los leads del CRM (`core-email`); (6) 💬 responder comentarios IG con propuesta IA + OK Telegram; (7) 📅 calendario de temporada hostelera sembrado (VeriFactu enero, terrazas primavera…); (8) 📖 Story de reciclaje al día siguiente («¿Te perdiste esto?»).
...
- **✅ prueba de diseño "Tremor-look" en el dashboard de plataforma (02/07/2026, PR de esta sesión).**
  ...
  - **FASE 3 — "paquete moderno" transversal (misma fecha, PR de esta sesión), preparación ANTES del rollout a todas las páginas:** (1) **Inter** vía `next/font` (self-hosted, `var(--font-inter)` en `globals.css`). (2) **Tokens semánticos** — `--positive/--negative/--warning/--info` + su `-bg`; `dashboard/ui.tsx` y la home los consumen (cero hex semánticos inline). (3) **MODO OSCURO automático** (`prefers-color-scheme: dark` redefine los tokens; fondos semánticos translúcidos; themeColor PWA dual). ⚠️ Las demás páginas tienen hex claros hardcodeados → en oscuro se verán imperfectas HASTA su pasada de rollout (la corrección es parte del despliegue página a página). (4) Paleta light refinada (bg `#f6f7f9`, radius 14, sombras en capas + `--shadow-lift`), hover `.card-hover`, scrollbar/selection/focus-ring, y **Recharts adaptado por CSS** (`.recharts-cartesian-grid line` / `.recharts-cartesian-axis-tick text` — el CSS gana a los atributos SVG, así la rejilla sigue los tokens en oscuro). (5) Sidebar: hover `.nav-link`, radio 10, logo degradado índigo→violeta. (6) **Botón de tema** en el pie del sidebar (`ThemeToggle.tsx`: 🌗 Auto → ☀️ Claro → 🌙 Oscuro; `localStorage('theme')` + `html[data-theme]`, script anti-parpadeo en `layout.tsx`; la media query oscura lleva la guarda `:root:not([data-theme="light"])`). (7) **Veto al oscurecimiento forzado del navegador:** con el móvil en ahorro de batería, Chrome/Samsung Internet repintan la web a oscuro por su cuenta aunque el usuario elija Claro (lo reportó Alberto con captura). Fix doble: CSS `[data-theme="light"] { color-scheme: only light }` + `<meta name="color-scheme">` (viewport `colorScheme: 'light dark'`) que el toggle y el anti-parpadeo reescriben a `only light`/`dark` según la elección — `only light` es la señal estándar que esos navegadores respetan para no auto-oscurecer.
  - **FASE 2 — página entera (misma fecha, PR #693 mergeado + PR de rediseño):** Alberto pidió la prueba a página completa ("solo un logotipo no se nota"). Rediseñada TODA la home `/dashboard` con el sistema Tremor, **solo presentación, cero cambios de datos/lógica**: nuevas primitivas compartidas **`dashboard/ui.tsx`** (`cardStyle`, `CardHeader`, `Stat` con `DeltaBadge` ▲/▼, `ThinBar`, `BarListRow`, `LegendDot`; server-safe, sin hooks), **`EvolucionChart.tsx`** (la gráfica de barras CSS server-side → Recharts client con barras redondeadas emerald/rose, ejes limpios, tooltip-card con neto y fila de Stats del periodo), KPI bar → **grid de 4 KPI cards** independientes, Comparativa mes vs anterior → Stats con pastillas de delta, «En qué se va el dinero» → **BarList** estilo Tremor (barra tintada con etiqueta encima + % de peso), y Correduría/Apartamentos/Pendiente-OTA repasados con `CardHeader`+`Stat`+`ThinBar`. `CobrosPisosChart` refactorizado para consumir `./ui` (un solo sistema). Responsive del KPI grid: 2 col ≤768, 1 col ≤480. Patrón a seguir cuando se toque cualquier otra página de plataforma.
- **🔥 HOTFIX post-#686: `column m.fecha does not exist` — `/api/finanzas/comparativa` y `/api/finanzas/proyeccion` daban 500 en prod (02/07/2026, PR de esta sesión).**
  ...
  - **Fix:** 3 referencias `m.fecha` → `m.fecha_operacion`; query validada con SELECT real contra la BD antes de commitear. **REGLA para SQL de movimientos: la columna es `fecha_operacion` (o `fecha_valor`), `fecha` NO existe.**
  - **Docs/skills actualizados en el mismo PR** (petición de Alberto): `plataforma-maestro` (firma nueva de `compararDeclaracion` + bloque «Mi declaración» + contrato nuevo del API), `perfil-fiscal` (caveats del comparador y de la estimación fin de año), `apps/plataforma/CLAUDE.md` (bullet de estado del PR #686 + landmine `fecha_operacion`).
  - **2º hotfix (misma query, error encadenado):** arreglado el 42703, la misma query falló con `42883: operator does not exist: uuid = text` — a `detectarPatronesSQL` también le faltaba el **cast `${cuentaId}::uuid`** (convención de TODAS las raw queries del repo, p.ej. `lib/banca.ts`). Doble prueba de que el helper de patrones (PR #646) nunca corrió bien en prod. ⚠️ REGLA: en `$queryRaw`, los params comparados contra columnas uuid SIEMPRE con `::uuid` — y al validar una query a mano con literales NO se detecta (el literal castea solo; el parámetro de Prisma no).
- **✅ Fix comparativa conjunta/separada en `/finanzas/fiscal` — salía "a devolver" en ambas mientras arriba ponía "a pagar" (02/07/2026, PR #686 mergeado).**
  - Alberto reportó (capturas móvil): cabecera «Resultado final 3157 € A pagar» pero la card «Conjunta vs Separada» daba «Devuelven 4442/3184 €» y una «Cuota íntegra: −3184 €» negativa. **3 bugs en `compararDeclaracion()` (`apps/plataforma/lib/fiscal-deducciones.ts`) y su UI:**
  - **(1) Retenciones fantasma:** `retAlberto = baseAlberto * 0.15` estimaba las retenciones como 15% de TODA la base (46.160 → 6.924 €) cuando las reales son ~583 € (el 15% solo aplica a comisiones de correduría; el capital inmobiliario de los pisos no lleva retención). Esos ~6.300 € inventados volteaban ambas modalidades a "devolver". **Fix:** nuevo parámetro `retencionesTitular` (reales); el route pasa `resumen.correduria.retencionesEstimadas`.
  - **(2) Reducción conjunta duplicada:** el route pasaba `baseImponibleEstimada` (que YA lleva restados los 3.400 €) y la función los restaba otra vez → conjunta con −6.800 y separada con −3.400 que no le corresponde. El «Ahorro conjunta: 1258 €» de la captura era exactamente 3.400×37% (solo el doble descuento). **Fix:** nuevo campo `fiscal.baseImponibleSinReduccion` en `getResumenFinanciero` y la función documenta que la base debe llegar SIN reducción.
  - **(3) UI cruzada:** la card «Separada (total)» pintaba `separada.total` (el RESULTADO, puede ser negativo) en el hueco de la cuota → el sinsentido «Cuota íntegra: −3184 €». **Fix:** cuota = `titular.cuota + conyuge.cuota` (≥0) y etiqueta corregida a «Cuota líquida» (es la cuota tras deducciones no reembolsables).
  - +4 tests de regresión en `lib/fiscal-deducciones.test.ts` (9/9 verdes con `node --test`). Sigue siendo orientativo: la cabecera y la comparativa pueden diferir legítimamente (la comparativa suma el rendimiento de Pilar y sus retenciones; la cabecera solo modela a Alberto).
  - **Caveat documentado sin corregir:** en separada, los mínimos por descendientes se quedan 100% en el titular (en la realidad van 50/50) y la maternidad se asigna al titular — el TOTAL separada no cambia, pero el reparto titular/cónyuge es aproximado.
  - **2ª iteración (misma sesión, pedida por Alberto): página simplificada a su pregunta operativa** «¿cómo voy ahora y cómo acabaría el año, solo o en conjunta, y me interesa meter más gastos?»: el bloque «Conjunta vs Separada» (botón "Ver comparativa") se sustituyó por **«🧾 Mi declaración»** que carga solo: dos cards **📍 Hoy** y **🔮 Fin de año (estimación)**, cada una con filas **👤 Solo yo** y **🤝 Conjunta con Pilar** (✓ mejor), + caja verde de palanca de gasto («cada 1.000 € de gasto ⇒ ~X € menos de cuota» + gasto necesario para bajar de tramo con la base PROYECTADA y meses restantes + aviso de que NO hay efecto acantilado entre tramos). Cambios: `lib/proyeccion-fiscal.ts` (nuevo — `getProyeccionFiscal()` extraído del route de proyección, reservas futuras sivra + patrones recurrentes), `/api/finanzas/proyeccion` refactorizado a ese helper (misma respuesta), **`/api/finanzas/comparativa` cambió su contrato**: ahora devuelve `{hoy, finAnio, bases, palanca, mesesRestantes}` (antes devolvía la ComparativaDeclaracion a pelo; su único consumidor era FiscalPageClient, actualizado).
- **🎉 PRIMER REEL IA PUBLICADO EN INSTAGRAM (02/07/2026) + fix del webhook de Telegram (PRs #685/#688) + marca sobreimpresa (PR #689).**
  - **LANDMINE resuelta — el webhook del bot de Telegram apunta a PLATAFORMA** (`/api/sivra/mensajes/telegram-webhook`), no a ia-rest: los callbacks `ig_*` del agente Instagram se descartaban en silencio (botones eternamente en "conectando…"; llevaban rotos desde que el webhook se movió a plataforma). Fix en 2 pasos: **#685** — plataforma reenvía `ig_*`/`blog_*`/`briefing_*` y mensajes `/ig …` a `https://www.iarest.es/api/telegram/instagram-callback` (maxDuration 300); **#688** — el reenvío se autentica con la cabecera `x-operador-secret` = `OPERADOR_SHARED_SECRET` (el `TELEGRAM_WEBHOOK_SECRET` de cada proyecto Vercel tiene un valor DISTINTO → el header reenviado daba 401). Cualquier callback nuevo de ia-rest debe entrar por este reenvío.
  - **Flujo completo verificado por Alberto**: cron → borrador «Maximiza ganancias» → 🔄 Comprobar (Cloudinary + título sobreimpreso) → ✅ Publicar → **publicado en Instagram** (`estado='publicado'`). Feedback: le gustó el vídeo pero faltaba la marca → **PR #689**: `videoConSubtitulo()` quema también una pastilla roja "ia.rest" arriba (los modelos de vídeo no saben escribir texto; el prompt lo prohíbe a propósito).

- **✅ Reels IA — mejoras post-E2E: subtítulo Cloudinary, 10s, auto-caducidad (02/07/2026, PR #683 mergeado).**
  - **Prueba E2E disparada en prod vía pg_net** (request 322 en `net._http_response`): `GET https://www.iarest.es/api/cron/instagram?manual=1&formato=reel` → `{ok, formato:'reel_ia', borradorId:'d6ab10cf-…'}`, borrador «Maximiza ganancias» en `estado='generando'` con `video_job`. Queda el 🔄→✅ de Alberto en Telegram. (El proxy de la sesión bloquea iarest.es y el relay `web_fetch_vercel_url` de Vercel daba 502 — **pg_net con `timeout_milliseconds := 110000` es la vía que funciona** para disparar URLs de prod desde la sesión; el default de 5s corta la petición.)
  - **Fix en prod (migración MCP):** el CHECK de `instagram_borradores.estado` solo admitía pendiente/aprobado/descartado → ampliado con `'generando'` y `'publicado'` (bug silencioso preexistente: el callback ya escribía 'publicado').
  - **`videoConSubtitulo()`** (`apps/ia-rest/src/lib/ai-video.ts`): al pulsar 🔄 y estar el vídeo listo, copia el MP4 de fal.ai a Cloudinary (las URLs de fal caducan en días; subida por URL remota con basic auth, patrón validado de ig-reel) y devuelve la URL con el **título sobreimpreso** abajo (`l_text:Arial_64_bold_center`, el 80% ve Reels sin sonido). Best-effort: si falta config o falla, se usa la URL de fal.
  - **Duración 10s** en los Reels del cron (`startVideoIA(prompt, {duration: 10})`) y **auto-caducidad**: al inicio del cron, borradores `generando`/`pendiente` >48h (sin `scheduled_for`) → `descartado`.
  - Ideas mayores anotadas y NO construidas: voz en off TTS, i2v con capturas reales del producto, aprendizaje por métricas de alcance.
- **✅ CUPS de luz → piso: mapeo persistido en la skill `facturas-correo` + 8 cargos de Endesa imputados (02/07/2026, solo datos + doc).**
  - Alberto subió a Drive **`CUP electricidad NUEVO.pdf`** (`FACTURAS Apartamentos/2026`, fileId `1iFpKQHHoY2JvdwOq-8TgBEJCy_5EfxiJ`): los 4 suministros de los pisos ahora a nombre de Alberto (antes Punto y Coma SL, xlsx 2024). Nueva sección **«Patrón especial — LUZ por piso (CUPS → apartamento)»** en la skill con la tabla CUPS/dirección/piso/`propiedad_id`/contrato Endesa/cuenta de cargo.
  - Contratos Endesa (altas feb-2026): `130139486193` = **Socorro 24** ✅ (confirmado por los correos «Hola Pilar… SOCORRO 24» ref P26CON…), `130139482171` = **Dúplex** ✅ (ya conocido), y los dos de Bustos Tavera: `130139685932` = Busto Reform (IZDA) / `130139655504` = Luxury Busto (DCHA) — deducidos por correlación factura↔ocupación (2 periodos coherentes) y **CONFIRMADOS por Alberto en la misma sesión («ES OK»)**; ya no hace falta verificación posterior.
  - **Imputados en `movimientos_bancarios`** (`propiedad_id` + `requiere_revision=false` + nota en `comentario`): 4× Socorro (`prop_house_sevillana`: −66,98 mar · −53,37 abr · −49,40 may · −53,93 jun) y 4× Bustos (Reform −71,42 y −133,71; Luxury −38,54 y −100,00). Todos Kutxa ****0855, `destino='turistico_pisos'` (ya confirmado antes).
  - **Inventario COMPLETO de CUPS en la skill** (2ª tanda, misma sesión): además de los 4 de pisos, tabla de «otros CUPS» con su tratamiento — Monte Carmelo (vivienda habitual, Energía XXI 130138945299 → `personal`), Sanlúcar (`personal`), San Luis ×3 (ex-suministros de Punto y Coma SL → cuenta de la SL, no conciliar en Kutxa/BBVA) y Espartinas (tercero, descartar). Historial de comercializadoras Fenie→Total→Endesa anotado.
  - Gmail: etiqueta **`Luz pendiente 2026`** (`Label_12`) documentada en la skill; quitada (+`Procesada`) de los 2 correos Endesa Socorro ya imputados. Los de **TotalEnergies** siguen pendientes: son de los contratos VIEJOS de la SL y se cargan a la cuenta de la SL, que NO está en `movimientos_bancarios` (sus PDF tampoco están en `_buzon_pdf` — el Apps Script arrancó el 22/06 y esos correos son de abril/inicios de junio).
- **✅ limpieza raíz Drive `FACTURAS Apartamentos/2026` + Pepephone→correduría (02/07/2026, solo datos/Drive).**
  - Revisión completa de los 21 ficheros sueltos de la raíz: **20 borrables** (todos con copia archivada en su mes o EMASESA de 2025 ya presentado). Detalle: 9 copias repetidas de la EMASESA 2026 Busto Reform 57,09 € (archivada como `2026-05-25_EMASESA-BustoReform_57.09EUR.pdf` y **conciliada** con el cargo Kutxa del 25/05, ref EMASEPE2600946516), 4 EMASESA 2025 de PyC SL (ejercicio presentado), 4 Endesa Dúplex ya archivadas, CREATE y Dimitri ya archivadas, CamScanner Leroy ya procesado. **NO borrar** `CUP electricidad NUEVO.pdf` (no es factura: listado de CUPS/contratos de luz por piso).
  - **PEPEPHONE = correduría** (decisión de Alberto): fibra+móviles de **San Juan de la Palma 28**. Las 6 facturas (ene–jun, ~60 €/mes) archivadas como `2026-0X-01_Pepephone-correduria_XX.XXEUR.pdf` en sus meses; regla `PEPEPHONE→seguros` sembrada en `banca_destino_reglas`; skill `perfil-fiscal` actualizada. ⚠️ **Los cargos Pepephone NO están en ninguna cuenta del sistema** (ni por nombre ni por importe) → se pagan desde una cuenta externa no conectada; conciliación pendiente de saber cuál (preguntado a Alberto).
- **✅ nueva skill programada `github-vigia` — vigía mensual del ecosistema GitHub/OSS (02/07/2026, PR de esta sesión).**
  - Nace de la sesión sobre Open_Motion: Alberto preguntó si "cosas así" se pueden conectar (sí: la ingesta GPS agnóstica ya lo cubre; Open_Motion es una librería C de fusión IMU que viviría en el firmware del tracker, no en nuestro stack) y pidió repos que nos ayuden a mejorar + un agente que vigile las actualizaciones.
  - **Hallazgo de la búsqueda (vigente):** VROOM (planificador automático de rutas pendiente en transporte), OSRM (ETA real por carretera + map-matching para km reales; hoy `etaMin` es línea recta) y openrouteservice (API hosteada gratis: rutas + optimización + geocoding, para validar sin infra). Recomendación: empezar por ETA real + geocoding de paradas.
  - **La skill** (`.claude/skills/github-vigia/SKILL.md`, diseño en `docs/superpowers/specs/2026-07-02-github-vigia-design.md`): 3 patas mensuales — releases de repos vigilados, descubrimiento por vertical juzgado contra pendientes, y npm outdated/CVEs. Estado entre ejecuciones en **`docs/VIGIA-OSS.md`** (nuevo). Telegram vía `/api/internal/alerta` si algo merece ojo; PR draft solo para bumps pequeños y seguros. ⚠️ Repos externos SIEMPRE por WebFetch/WebSearch (el MCP de GitHub va scopeado a `central`).
  - **Pendiente manual de Alberto:** crear el trigger (rutina 9, mensual día 15 ~07:00, prompt `Ejecuta la skill github-vigia` + `PLATAFORMA_URL`/`CRON_SECRET` en instrucciones) — anotado en `docs/RUTINAS-PROGRAMADAS.md`.
  - **PRIMERA PASADA ejecutada (misma sesión, 02/07):** baseline de versiones fijada en `VIGIA-OSS.md`. Hallazgos que requieren decisión (detalle y links en la bitácora): **npm audit 3 high** — `xlsx` en ialimp (SheetJS abandonó npm; migrar a cdn.sheetjs.com ≥0.20.2 o exceljs), `nodemailer` 8→9.0.1 en rrhh, `fast-xml-parser` 4→5.7 en plataforma (parsea XML externo de PLACSP en concursos-radar). Ningún bump auto-seguro (todos major/migración). Añadido `zarpilla/verifactu-node-lib` (JS/TS, inmaduro) a vigilados como referencia de `core-fiscal` — VeriFactu obligatorio autónomos desde 01/07/2026. **Landmine corregido en la skill:** `api.github.com` está capado por el proxy en rutinas → usar páginas `github.com/*/releases/latest` y `registry.npmjs.org`.
...
- **✅ PIPELINE Reels IA → Instagram COMPLETO (02/07/2026, PR #677 mergeado).**
  ...
  - Fallbacks intactos: vídeo IA falla → reel slides Cloudinary → imagen. Escape `?video=0`.
  - Nuevo `apps/ia-rest/src/lib/ai-video.ts` (cliente EF: `startVideoIA`/`checkVideoIA`).
  - Prueba manual: `GET /api/cron/instagram?manual=1&formato=reel` → mensaje Telegram → 🔄 → ✅.
...
- **✅ mejoras de cierre del ciclo bancario/facturas (02/07/2026, PR #671 MERGEADO con build de plataforma verde verificado por API).**
  ...
  - **Skill `facturas-correo`, Paso 1-bis**: la pasada diaria también revisa los ficheros SUELTOS en la raíz de `FACTURAS Apartamentos/2026` (subidas manuales tipo CamScanner) y los procesa como candidatos (leer→clasificar→copiar a mes→conciliar), con idempotencia por nombre normalizado. También corregida su regla vieja «mobiliario→amortizar» (ahora apunta a la canónica de `perfil-fiscal`).
  - **`auditoria-diaria`**: nuevo check en el paso 4 — contradicciones entre reglas DICTADAS por Alberto en skills vs memoria = hallazgo 🔴 (la copia divergente se alinea con lo dictado más reciente). Nace del incidente amortizable de hoy.
  - Convención anotada: los extractos de tarjeta pueden llegar en Excel o PDF; ambos entran por `/banca` con dedupe.
- **✅ rendimiento `/finanzas/gastos` — buckets con montaje perezoso + paginación (02/07/2026, PR #666 MERGEADO).**
  - Además se persistió la lección: **nueva regla global "Rendimiento UI" en el `CLAUDE.md` raíz** (junto a la de Responsive — listas largas = montaje perezoso + paginación + sin loader que desmonte) y nota de patrón de referencia en el skill `plataforma-maestro` (sección Sidebar Finanzas). `GastosTab.tsx` es el patrón a copiar.
...
- **✅ ENERGIA XXI = luz de Monte Carmelo → personal NO deducible (02/07/2026, rama `claude/energia-xxi-deduction-vsia40`).**
  - Decisión de Alberto (screenshot de la bandeja de Gastos): los recibos «RECIBO ENERGIA XXI COMER…» de Kutxa son la luz de la **vivienda habitual** (Energía XXI = comercializadora regulada de Endesa) → `personal`, auto-confirmado.
  - **Código**: regla determinista `/ENERGIA\s+XXI/` en `apps/plataforma/lib/destino.ts` (personal + `confirmado:true`, cualquier banco y signo — evita también que caiga a `seguros` por descarte en BBVA) + test en `destino.test.ts` (18/18 verde).
  - **BD (ya aplicado en prod)**: regla `ENERGIA XXI → personal` en `banca_destino_reglas` (cuenta de Alberto) + los 11 cargos existentes (mar–jun 2026, −587,06 €) confirmados (`destino_confirmado=true`, `requiere_revision=false`). SQL versionado: `prisma/sql/2026-07-02_energia_xxi_personal.sql`.
  - **Agente `facturas-correo`**: nota explícita en el Paso 2 — correos de ENERGIA XXI → personal, no archivar/conciliar; no confundir con ENDESA ENERGIA (pisos) ni TE/Endesa del dúplex.
  - ⚠️ Clave de regla ESPECÍFICA a propósito: «ENERGIA» a secas arrastraría los conceptos «ENDESA ENERGIA» de los pisos (misma lección que «ELECTRICIDAD», 23/06).
- **✅ tarjeta Kutxabank de Pilar (4662032019650302) importada a `movimientos_bancarios` (02/07/2026, solo datos — sin cambios de código).**
  ...
  - **Dudas RESUELTAS por Alberto (02/07/2026, misma sesión):**
    - **IKEA 296,93 €** y **Leroy 192,39 € − dev. 140,64 €** → `turistico_pisos` confirmado, **gasto deducible del AÑO al 100%** (⚠️ IKEA se marcó `amortizable` por error siguiendo la regla vieja de la skill `perfil-fiscal`; Alberto lo revirtió en la sesión: «NO SE AMORTIZA NINGUNA FACTURA SIN QUE YO LO DIGA — la idea es todos los años meter los máximos gastos posible». Ya estaba como regla permanente en la entrada del 30/06; la skill `perfil-fiscal` era la que contradecía y quedó corregida en este mismo push). Ambos como **gasto GENERAL de pisos** (sin `propiedad_id`, decisión de Alberto).
    - **Resto (TEKNOBELL 400 €, PRIMAPRIX ×2, Ferretería Echevaria, Hostepro, AUSSA, Hdad. Rocío Triana)** → **personal NO deducible, confirmado** (sin mecenazgo en la Hermandad).
    - **Regla `PRIMAPRIX→seguros` ELIMINADA** de `banca_destino_reglas` (Primaprix es súper de descuento, no gasolinera — la confusión era con PETROPRIX, cuya regla sigue viva). Los 3 cargos Excel que había mandado a correduría reclasificados a personal confirmado. Los Primaprix de la tarjeta PSD2 de Alberto ya estaban en personal confirmado.
    - Verificado el **cuadre mensual** detalle-tarjeta ↔ cargo `TARJ.CRDTO` en la corriente ****0855: 0,00 € de diferencia los 6 meses. Anti-duplicado: 0 gemelos activos (el solape Excel↔PSD2 de la tarjeta 750300 ya estaba saneado por las guardas).
  - **📬 AVISO PARA `facturas-correo` (pendiente):** Pilar reenvía por email la **factura de IKEA (296,93 €, compra 02/06/2026)** → al verla: archivar en Drive (`2026/06-Junio-2026`) como deducible de pisos y **conciliar** con el movimiento de la cuenta `💳 Tarjeta Kutxabank Pilar` (`cuenta_bancaria_id b8c4376f-cee9-40b7-a447-a0a2345a1b75`).
  - **✅ Leroy Merlin YA conciliada (02/07/2026, en sesión):** Alberto subió el escaneo a mano a la raíz de `FACTURAS Apartamentos/2026` (CamScanner; factura 009-0006-854437 de 192,39 € — papel pintado + cola, confirma reparación del año — y rectificativa R019179 de −140,64 €). Copiada como `2026-06-19_leroymerlin_192.39_y_rectificativa_-140.64.pdf` a `06-Junio-2026` y los 2 movimientos marcados `conciliado=true` + `factura_ref`. El original de la raíz lo borra Alberto (el MCP de Drive no mueve, solo copia). ⚠️ Recordatorio operativo: `facturas-correo` escanea GMAIL — lo subido a mano a Drive NO lo detecta solo; mejor reenviar facturas por email.
- **✅ video IA Instagram — FUNCIONANDO en producción, flujo ASÍNCRONO (02/07/2026, PRs #656, #658 y #661 mergeados).**
  ...
  - **Flujo ASÍNCRONO final (PRs #661/#670)**: el flujo síncrono moría por timeouts. EF `ig-video-gen` con 2 acciones: `action=start` (encola en fal.ai, devuelve `requestId`+`statusUrl`+`responseUrl` al instante) y `action=status` (consulta con esas URLs; `COMPLETED` ⇒ `videoUrl`). Endpoint: `GET /api/ig-ai-video?tipo=X` → `{ok, jobId, consultar}` en ~2s; el campo **`consultar`** lleva `s=`/`r=` (status/response URLs urlencoded) y ES la URL de polling — usar esa, no solo el jobId.
  - **GOTCHA fal.ai #1 — polling**: NUNCA reconstruir a mano las URLs de la cola; usar SIEMPRE `status_url`/`response_url` que devuelve fal al encolar (los modelos anidados usan el app-base `fal-ai/kling-video`, no la ruta completa). La EF valida host `queue.fal.run`.
  - **GOTCHA fal.ai #2 — modelo**: `fal-ai/kling-video/v2.1/standard/text-to-video` NO existe (v2.1 standard es solo i2v). El alias ACEPTA el submit (devuelve request_id) y el worker muere con `Path not found` que solo asoma al leer el resultado. Modelo final: **`fal-ai/kling-video/v2.5-turbo/pro`** (t2v e i2v, EF v6, ~0,35$/vídeo). WAN (`fal-ai/wan-t2v`) quedó descartado: vídeos casi estáticos.
  - **Prompts = Reels de PRODUCTO (PR #667)**: tipos `voz`/`cocina`/`qr`/`datos`/`servicio`/`cobro` — escenas con movimiento explícito de iarest mejorando la gestión del bar (comandar por voz, KDS, pedido QR, analítica, cobro móvil). Regla: los prompts de vídeo DEBEN describir movimiento o el modelo genera una foto.
  - **✅ VERIFICADO con Kling (02/07/2026, Alberto)**: `tipo=voz` → jobId en 2s → `COMPLETED` en ~45s → MP4 9:16 correcto (camarero con tablet en bar de tapas).
