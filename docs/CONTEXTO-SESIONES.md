...
- **🔁 `mercado-booking`: 5 pasadas seguidas gastando el cupo diario en repetir un objetivo ya
  cumplido (07/09/2026).** La línea `PRIORIDAD TEMPORAL` del trigger programado pedía llegar a ≥3
  comparables en ≥3 fechas de jul/ago-2027; se cumplió el 03/09 y las pasadas del 04/09, 05/09 y
  07/09 lo han vuelto a confirmar sin novedad, quemando las 24 ventanas/día en la misma comprobación
  mientras el resto del plan (568 candidatas) se queda sin medir. Ninguna sesión puede editar el
  prompt del trigger (`CronList` solo ve jobs in-memory propios) — **hace falta que Alberto quite
  a mano la línea desde la UI del trigger programado.** Detalle en `docs/AGENTES-BITACORA.md`.
