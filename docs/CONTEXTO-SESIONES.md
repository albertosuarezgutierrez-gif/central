...
- **🧹 DEDUPE BANCARIO CROSS-ORIGEN (Excel↔PSD2) + facturas DIGI 2026 apuntadas — rama `claude/digimobil-email-accounting-rib70e` — 26/06/2026**
  Alberto preguntó si el agente `facturas-correo` contabiliza el gasto de DIGI (`digimobil.no-reply@bulkmail.digimobil.es`). Al revisar: **6 facturas DIGI 2026** (ene–jun, ~76€/mes a *Punto Y Coma Gestion SL*, internet pisos = `turistico_pisos` deducible). Faltaban apuntes en Drive de ene/mar/jun → **creados** los 3 Google Docs `YYYY-MM-DD_digi_76.00EUR` en `FACTURAS Apartamentos/2026/MM-Mes-2026` (feb/abr ya tenían PDF, mayo ya tenía doc).
  - **HALLAZGO GORDO:** al conciliar contra `movimientos_bancarios` aparecieron **duplicados masivos**. Causa (diagnóstico de Alberto, correcto): se cargó un **Excel** y luego se conectó el **banco (PSD2)**; el mismo movimiento entra por las dos vías con el **concepto distinto** (banco verboso `…FACTURA DIGI`, Excel truncado `RECIBO DIGI SPAIN TELECO`), y como el `dedupe_hash` es **por contenido**, el `ON CONFLICT (cuenta_bancaria_id, dedupe_hash)` no los colapsa → **143 filas duplicadas** (138 cross-origen psd2↔xls-kutxa + 5 internas) inflando el P&L en **+41.762,85€ de ingreso fantasma y +11.872,60€ de gasto fantasma**.
  - **SANEADO (reversible, soft):** marcadas `duplicado_estado='ignorado'` conservando SIEMPRE el feed del banco (`psd2`), propagando `destino_confirmado` al superviviente, sin tocar repeticiones legítimas (3 Bizums de 61,25€, "casa"/"casa rural", etc.). Activas: 1527→1384 filas; gastos −191.473,95→−179.606,35; ingresos +234.599,61→+193.062,26. SQL documentado: `apps/plataforma/prisma/sql/2026-06-26_dedupe_cross_origen.sql` (incluye el UPDATE para deshacer).
  - **ENSEÑADO AL AGENTE (prevención):** guarda anti-duplicado cross-origen en `lib/banca.ts::importarExtracto` (tras cada import de Excel marca `ignorado` las filas con gemelo PSD2 por `(cuenta,fecha,importe)`, capado al nº de gemelos). Docs: LANDMINE en `apps/plataforma/CLAUDE.md` + Paso 4 de la skill `facturas-correo` (conciliar SOLO el `origen='psd2'`, filtrar `duplicado_estado<>'ignorado'`).
  - **BARRIDO RECURRENTE (skill):** `facturas-correo` Paso 4·bis — en CADA pasada repesca los justificantes deducibles ya archivados cuyo cargo aún no tiene 📎 y los concilia cuando el movimiento entra (genérico, idempotente). Así la DIGI de junio se conciliará sola cuando se domicilie el cargo del 29/06.
  - **VISTA CANÓNICA `v_movimientos_activos`** (`prisma/sql/2026-06-26_v_movimientos_activos.sql`, YA aplicada por MCP): centraliza el filtro `duplicado_estado<>'ignorado'`; toda lectura nueva de saldo/P&L debe leer de aquí (causa raíz = consultas que "olvidaban" excluir duplicados). Documentada en el LANDMINE de `apps/plataforma/CLAUDE.md`.
  - **AUDITORÍA REFORZADA** (`/auditoria-diaria`): paso **4-bis** "agentes programados al día" (drift skill↔código/BD + pendientes recurrentes capturados + query de cordura por agente), paso **2-ter** "duplicados cross-origen = 0" (vigila que la guarda de ingesta no falle), paso **7** aviso por **Telegram** (curl al bot del monorepo) solo en hallazgos 🔴.
  - **RUTINAS confirmadas ACTIVAS por Alberto:** auditoría diaria 04:00, facturas-correo 08:00, auditoría semanal dom 04:00, pricing-agente lun 07:00 (doc `RUTINAS-PROGRAMADAS.md` actualizado: pricing pasó de "pendiente" a "activa"). Solo `fiscal-novedades` queda pendiente de trigger.
  - **CONTROL DE RECURRENTES OPERATIVO (skill):** la tabla `facturas_drive` estaba VACÍA → el semáforo `/sivra/facturas-control` y la alerta `facturasFaltantes` no funcionaban. Añadidos a `facturas-correo`: **Paso 3·bis** (al archivar, registra en `facturas_drive` con el `id` de `PROVEEDORES_RECURRENTES`) y **Paso 6** (cada pasada compara esperadas vs recibidas y lista «recurrentes que faltan este mes»: 1 lavandería, 1 limpieza, luz de cada piso, agua bimestral…). Maneja cambios de proveedor (Socorro luz TotalEnergies→ENDESA; lavandería cambió a finales 2025). **Confirmado por Alberto (29/06):** son **4 apartamentos** — `Socorro` (= **House Sevillana**, MISMO piso), `Luxury`, `Bustos Reform`, `Dúplex`; el **agua del Dúplex va dentro de la comunidad** (no se espera factura EMASESA aparte). **Indicador en pantalla + aviso por Telegram** (bot del monorepo) cuando falte algún recurrente del mes.
  - **Reclamación de facturas (borradores Gmail creados):** TotalEnergies/Socorro 2025 (abr/may/jun/nov, a `atencionalcliente@totalenergies.com`). El de Giraldillo 2025 se descartó: Giraldillo empezó en 2026; la lavandería de finales 2025 era OTRO proveedor (pendiente identificar quién cobró los 514,46€ del 23/12/2025). ENDESA no tiene buzón (no-reply); sus facturas ya llegan por email/Área Cliente.
  - **PENDIENTE:** la factura DIGI de **enero** no tiene cargo en el banco (extracto de enero sin importar); **junio** aún no ha pasado (cargo 29/06). Conciliación 📎 de las DIGI no ejecutada — el barrido del Paso 4·bis la cerrará. Todo el trabajo va en el **PR #541** (draft).

- **🧳 AGENTE HUÉSPED SIVRA: consigna de equipaje AHORA POR ZONA (punto físico concreto por piso) — rama `claude/agente-huesped-consigna-zona` — 26/06/2026**
