...
- **⏰ El trigger de `mercado-booking` sigue con la línea `PRIORIDAD TEMPORAL` obsoleta — 4ª pasada repitiendo un objetivo ya cumplido (13/09/2026).** El objetivo (≥3 comparables en ≥3 fechas de jul/ago 2027 por piso) se confirmó cumplido el 03, 04 y 05/09; hoy, tras 8 días sin pasadas de este agente, el trigger volvió a disparar la misma instrucción acotada y gastó el cupo diario repitiendo la comprobación. Ninguna sesión puede editar el prompt de un trigger programado: **pendiente de que Alberto la quite a mano desde la UI del trigger.** Detalle en `docs/AGENTES-BITACORA.md` (entradas 03/09, 04/09, 05/09, 13/09).
