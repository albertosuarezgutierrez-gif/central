...
- **🚚 UI de FLOTA en el panel del dueño de ia-rest — 26/06/2026 (rama `claude/jj-logistica-materiales-k5eko3`)**
  Segundo consumo de `module-flota`: pantalla `/owner/flota` (cliente, solo lectura) que consume el endpoint `GET /api/owner/flota/resumen` (#534) y muestra **rentabilidad por vehículo** (KPIs vehículos/portes/coste estimado/real + tabla con km, estimado, real y **desviación** coloreada rojo/verde, badge propio/externo). Nav: nueva pestaña **Flota** en `GRUPOS` de `owner/page.tsx` (icono `truck` añadido a `ICONS`), `href: '/owner/flota'`, gated por `modulo: 'eventos'` (la flota sirve a catering/eventos → no requiere toggle nuevo; se ve cuando eventos está activo, como en el tenant Catering JJ). Aditivo: no toca rutas ni datos existentes; con flota vacía muestra estado "sin portes". Verificación: `tsc` 0 errores en mis archivos (los restantes son symlinks/deps de entorno que CI resuelve). Mergeable solo (app viva, cambio additive read-only) tras typecheck+build verdes.
