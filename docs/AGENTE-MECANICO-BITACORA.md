...
- (23/09/2026) dos componentes de UI del expediente de anulación (`AnulacionPoliza.tsx`, `Anulaciones.tsx`) + montarlos en `poliza/[id]/page.tsx` y `HoyCockpit.tsx` — agente-mecanico — fallo: no escribió ningún fichero (árbol limpio) y devolvió un informe describiendo el código como si existiera, con «Pendiente del usuario: copiar el código». Sin salida de tsc pegada. Lo hizo la sesión.
- (24/09/2026) parte de plataforma del registro de quejas del SAC (lib + ruta proxy + `Quejas.tsx` + test) — agente-mecanico — fallo: construyó todo contra un contrato INVENTADO (copió supresiones: estados `abierta/rechazada`, campos `asunto/identidadId`) que no casa con el puerto real; además dejó un duplicado en una carpeta literal `\(usuario\)` y el test fuera de `test/`. tsc verde sobre código que nunca habría funcionado. Reescrito por la sesión.
- (24/09/2026) hex→tokens en 7 ficheros de la correduría + `tabla-polizas` en `CuadreComisiones`/`Recaptacion` — agente-mecanico — ok con retoque: cambió el texto blanco sobre fondo de color (`#fff`) por `var(--surface)`, que en oscuro sale oscuro; la sesión lo devolvió a `#fff` (convención de `btnStyle('primario')`). tsc y tests pegados.
