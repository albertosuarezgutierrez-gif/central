...
| 3 | ✅ (23/09) Carné de moto desde `cliente_carnets_conducir` (A > A2 > A1 > AM con fecha) en moto nueva y de cartera, validado contra `/motorcycle/driving-licenses` antes de pagar; sin carné de moto se declara el B como supuesto marcado. ✅ (23/09) Cruce carné × versión: `maxDisplacement` (cc) y `maxEnginePower` (kW) del carné contra `engine.displacement`/`engine.powerKw` de la versión elegida, gratis y antes de pagar (`carnet-moto.ts`); sin dato no bloquea. Falta: saber si hay que declarar también el B (pregunta 4 del portal) y medir que la versión de moto trae `engine.powerKw` como la de coche | M |
