...
| 3 | ✅ (23/09) Carné de moto desde `cliente_carnets_conducir` (A > A2 > A1 > AM con fecha) en moto nueva y de cartera, validado contra `/motorcycle/driving-licenses` antes de pagar; sin carné de moto se declara el B como supuesto marcado. Falta: cruzar con `maxDisplacement`/`maxEnginePower` de la versión y saber si hay que declarar también el B (pregunta 4 del portal) | M |
