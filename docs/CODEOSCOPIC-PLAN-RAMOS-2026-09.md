...
| 2 | ✅ (23/09) `garajes-moto` (`/motorcycle/garage-types`), `carnets-moto` y `/motorcycle/registration-date` para la fecha de la matrícula; plataforma pide los garajes de moto. Sin usar aún: `/motorcycle/insurance-companies`, `engine-types`, zonas de carné propias | S |
| 3 | ✅ (23/09) Carné de moto desde `cliente_carnets_conducir` (A > A2 > A1 > AM con fecha) en moto nueva y de cartera, validado contra `/motorcycle/driving-licenses` antes de pagar; sin carné de moto se declara el B como supuesto marcado. ✅ (23/09) Cruce carné × versión: `maxDisplacement` (cc) y `maxEnginePower` (kW) del carné contra `engine.displacement`/`engine.powerKw` de la versión elegida, gratis y antes de pagar (`carnet-moto.ts`); sin dato no bloquea. ✅ (23/09) El B de la ficha viaja DETRÁS del de moto (`fechaCarnetB`), como el ejemplo oficial `motorcycle/quotation-request.json` (B + A); sin carné de moto el principal ya es el B. Sigue abierta para el portal si alguna compañía lo EXIGE (pregunta 4) y medir que la versión de moto trae `engine.powerKw` como la de coche. 📏 Medir: con sesión de plataforma, `/api/correduria/codeoscopic-crudo?tipo=carnets-moto` (límites reales) y `?tipo=versiones-moto&marcaId=…&modeloId=…&motor=Gasoline` (si trae `engine.powerKw`); gratis. Pista sobre el B: el ejemplo oficial `motorcycle/quotation-request.json` declara B **y** A (`docs` del repo `asegura`, § 4.5) | M |
