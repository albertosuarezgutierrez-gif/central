...
| **Qué hace** | `trading-watchdog` comprueba TRES huellas de la pasada nocturna de trading: `broker_saldos` (NAV de IBKR), el latido `trading_analizar` (con `trading_tesis` como huella de respaldo — la tabla es idempotente, así que una 2ª pasada del mismo día no la mueve) y el latido `trading_puntuar` (el cierre: sin él, stops y walk-forward se quedan sin actualizar aunque NAV y tesis estén frescos). Avisa si falta cualquiera de las tres. `agentes-latido` (`lib/monitoring/latidos.ts`, registro `AGENTES_VIGILADOS`) comprueba, por cada agente vigilado, una huella FIABLE en BD que SOLO se refresca cuando ese agente corre — hoy vigila **pricing** (`market_rates prop_%`, 192h — es la sesión Claude SEMANAL, no el barrido), **correo-triaje** (`correo_cursor.updated_at`, 6h), **facturas-scan**, **ialimp_pms**, **sivra_eventos** y **sivra_mercado_sweep** (30h cada uno: son diarios). Nace de que el agente de pricing dejó de correr en silencio y una reserva entró un 40% bajo mercado sin que nadie se enterara. ⚠️ **Un latido mide FRESCURA, no CORRECCIÓN**, y confundirlo sale caro: el 01/08/2026 `market_rates` estaba fresquísima y aun así el ancla de House venía de comps de otro aforo — eso no lo caza un latido, lo caza el centinela #9 del guardián de precios. Al añadir un vigilante, pregúntate cuál de las dos cosas estás midiendo. |
