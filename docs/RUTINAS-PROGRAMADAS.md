...
### 11. Vigía de LLMs — *pendiente de trigger*
| | |
|---|---|
| **Cuándo** | Semanal, **lunes ~07:00 CEST** (después del pricing-agente de las 06:00) |
| **Prompt** | `Ejecuta la skill llm-vigia` (+ `PLATAFORMA_URL`/`CRON_SECRET` en instrucciones para el aviso, como psd2/github-vigia) |
...
| **Resultado** | Actualiza `docs/VIGIA-LLM.md` (modelos vivos/deprecados + candidatos + bitácora). Modelo cableado muerto/deprecado o gratis nuevo mejor → **Telegram**; swap seguro (id muerto→vigente) o plumbing de proveedor nuevo → **PR draft** `claude/llm-vigia-<fecha>`. Sin novedades → sin ruido. |
