...
| | |
|---|---|
| **Cuándo** | `.github/workflows/latido-reparar.yml`, `0 8 * * *` (15 min detrás del cron de latidos, con su parte ya escrito). También `workflow_dispatch`. |
| **Prompt** | *N/A* — corre como código. La orden al orquestador la construye el propio workflow con la EVIDENCIA (el `detalle` crudo del latido + los ficheros que lo escriben), **nunca** con la `nota` de `AGENTES_VIGILADOS`. |
| **MCPs / envs** | Secrets del repo: `PLATAFORMA_URL`, **`ALERTA_TOKEN` o `CRON_SECRET`** (reclamar/reportar — el workflow prefiere el estrecho y cae al maestro; los dos endpoints usan `isRoutineAuthorized`, que acepta ambos), `AI_GATEWAY_SECRET` (orquestador), `GH_PAT_TRIGGER` (opcional, para el PR). 🔑 **No intentes copiar el `ALERTA_TOKEN` de Vercel aquí: está marcado Sensitive** (escritura sin lectura, ni dashboard ni API lo devuelven) y **rotarlo desincronizaría todas las Rutinas de claude.ai/code**, que lo llevan escrito en su prompt — la avería del 19/07/2026. Por eso el respaldo es `CRON_SECRET`, que ya vive en los secrets del repo. |
| **Qué hace** | Cierra el hueco que dejaba el vigía: había quien DETECTA y nadie que REPARE (el `sivra_canal` roto del 19/08/2026 siguió tarificando con el ×1,20 hasta que un humano leyó el Telegram). Pide a `POST /api/internal/reclamar-reparacion` un agente reparable — solo lo son los partes con forma de **excepción** (`lib/monitoring/reparable.ts`: SQLSTATE, nombre compuesto de excepción o marcador de runtime); un «no he podido mirar» (IMAP caído, Serper vacío, conector mudo) NO se arregla tocando el repo y se descarta. Corre `scripts/ai-programar.mjs` y aplica el **gate de prueba**: el diff debe traer un `*.test.ts` que **falle sobre `main`** y **pase con el parche** (+ suite entera en verde). Solo entonces mergea solo. |
| **Frenos** | Una firma de error = un intento (un síntoma nuevo permite otro) · máx. 3 por agente en 30 días · un intento vivo a la vez (`concurrency` + claim en BD) · el diff NUNCA puede tocar `.claude/**`, `CLAUDE.md`/`AGENTS.md`, `.github/workflows/**` ni `.sql` · el coste pasa por `/api/ai/programar` (presupuesto diario + `ai_usos`). |
| **Resultado** | **El éxito es silencio.** Telegram SOLO si: no supo arreglarlo (PR draft) · lo mergeó y el latido **sigue rojo a las 24 h** (veredicto que dicta `agentes-latido`, no el reparador) · agotó los 3 intentos. Tabla `agente_reparaciones`. Diseño: `docs/superpowers/specs/2026-08-20-latido-autoreparacion-design.md`. |
