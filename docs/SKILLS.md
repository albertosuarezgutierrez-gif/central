...
| **`trading-analista`** | Pasada diaria del agente de inversión asistida sobre Interactive Brokers (Fase 1, SOLO paper trading — cero ejecución real). Lee cartera + watchlist, tira precios (IBKR MCP) y fundamentales (FMP) por MCP, llama a `/api/trading/{analizar,puntuar}` de plataforma y resume por Telegram. Compone el paquete puro `@central/module-trading`. **PENDIENTE de activar el trigger** (ver `docs/RUTINAS-PROGRAMADAS.md`): falta dry-run manual antes de programarla. |
