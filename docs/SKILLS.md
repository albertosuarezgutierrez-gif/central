...
| **`agente-correduria`** | El agente de Grupo ASegura (correduría). Fase 0: ciclo semanal de aprendizaje — cartera en vivo, vencimientos, novedades del sector (DGSFP/IDD/LCS), informe Telegram y conocimiento acumulativo en `references/sector.md`. NUNCA contacta clientes/compañías (borradores siempre). Fases futuras (emisión Avant2, cliente-facing) requieren OK explícito de Alberto. Rutina semanal (ver `RUTINAS-PROGRAMADAS.md` §21). |
| **`seo-asegura`** | SEO, contenido y **redes sociales** de la web pública de la correduría (`grupoasegura.es` / `apps/asegura-web`). Ejecuta las fases 3-4 de `docs/ASEGURA-MARKETING-PLAN.md`; backlog vivo en `docs/ASEGURA-SEO-REDES-IDEAS.md`. **No publica nada**: deja borradores (regla global de comunicaciones salientes). Nunca tarifica (Avant2 = 0,50 €/consulta). El copy pasa por `lib/ramos.test.ts`: prometer precio convierte la web en asesoramiento (RDL 3/2020). Complementa a `agente-correduria`, que lleva la cartera. Rutina semanal **pendiente de crear**. |
| **`correduria-crm`** | Router del CRM de la correduría (`/correduria` en plataforma): ficha cliente/póliza, edición, leads, relaciones y autorizaciones, portal del cliente, conciliación Codeoscopic↔CIMA. Léela ANTES de tocar la cartera. Visión y estado real en `docs/CORREDURIA-CRM-VISION.md` (dictado de Alberto, 02/09/2026). |
| **`cima-ingesta`** | La TUBERÍA que alimenta la cartera: CIMA/EIAC/TIREA de punta a punta (Actions → CRM → adaptador Java en Fly → TIREA), cuarentena, cobertura de campos, caja negra del webhook de Codeoscopic y el diagnóstico de «la ingesta está muda». Úsala antes de tocar cualquier eslabón: **confirmar un fichero a TIREA lo saca de la cola PARA SIEMPRE** y el adaptador vive en la cuenta de Fly de Manuel. Detalle en `docs/ASEGURA-CIMA-INGESTA-INVENTARIO.md`, `docs/CIMA-CUARENTENA.md` y `docs/ASEGURA-CIMA-COBERTURAS.md`. |
| **`mercado-booking`** | Medir el precio REAL por fecha y aforo con el conector de Booking y escribirlo en `market_rates` (`fuente='booking_mcp'`) — la única fuente de SIVRA que distingue temporada (el barrido por búsqueda web da precios de anuncio sin fecha). Rutina diaria **ACTIVA** («SIVRA mercado booking (diario)», cron `30 8 * * *` UTC — verificado contra `list_triggers` el 31/08/2026, con última pasada SUCCEEDED); a mano si Alberto pide refrescar comparables. |
| **`inversion-inmueble`** | Analizar si comprar UN inmueble concreto renta como VUT: mide el mercado por fecha y aforo con el conector de Booking, resuelve la puerta legal (licencia + nº de Registro Único), llama a `POST /api/inversion/underwrite` y devuelve veredicto con los DOS escenarios —entero y segregado—. **A demanda**, no programada: se dispara cuando Alberto pasa un anuncio. El cálculo vive en `apps/plataforma/lib/inversion/*` (puro y testeado); pantalla `/inversion`. Diseño en `docs/superpowers/specs/2026-08-27-agente-inversion-inmobiliaria-design.md`. |
