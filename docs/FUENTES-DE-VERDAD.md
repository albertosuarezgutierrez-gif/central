...
| `docs/TRASPASO-CORREDURIA.md` | `apps/seguros/**` (la vertical **aún no existe**: el día que aparezca, este runbook pasa de plan a histórico), `apps/plataforma/lib/correduria.ts` + `apps/plataforma/app/api/correduria/**` (la frontera que el doc marca: comisiones cobradas ≠ operativa del CRM). **PENDIENTE ABIERTO mientras siga BLOQUEADO en Fase 0** — esperando a que Alberto envíe el mensaje a Manuel Suárez |
