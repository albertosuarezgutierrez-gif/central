...
| **`trading-analista`** — agente de inversión asistida sobre Interactive Brokers, Fase 1 técnica cerrada + Fase B por SELECCIÓN en marcha (17-18/07/2026) | Paquete puro `@central/module-trading` (indicadores, torneo de estrategias, motor paper, factores value+quality+momentum, Piotroski/magic formula, riesgo/atribución). Endpoints `/api/trading/**` (analizar/puntuar/factores/gurus/fundamentales/insiders/seleccion/validar-oos/paper/saldo/descubrir/screener/fmp). Modelos Prisma `trading_*` + `BrokerSaldo` (RLS ON). Resumen por Telegram (`lib/trading-notify.ts`, `lib/trading/paper-tracker.ts`). Skill `.claude/skills/trading-analista`. **Sin autonomía hasta ser rentable; nunca ejecuta órdenes reales.** **Trigger CREADO y corriendo, bloqueado por infra** (falta `ALERTA_TOKEN` en su entorno + 403 de red hacia Vercel — ver `docs/RUTINAS-PROGRAMADAS.md`). |
