...
## Constantes centralizadas sivra (01/07/2026, PR #635)
- **`lib/sivra/constantes.ts`** — fuente única de: `CATEGORIAS_GASTO`, `PROPS_GASTO`, `PROP_NAMES_GASTO`, `PROPS_CALENDARIO`. Importar desde aquí en cualquier componente de pisos. NO copiar localmente.
- **`lib/portales.ts`** — fuente única de: `PORTAL_COLORS`, `PORTAL_LABELS`, `PORTAL_MAP`. Consumido por `sivra/income`, `sivra/calendario`, `lib/sivra/smoobu-sync.ts`. **Excepción**: `sivra/mercado` tiene su propio `PORTAL_COLORS` con claves lowercase e incluye Tripadvisor — es intencional (mercado externo ≠ portales de ingresos propios), NO unificar.
- **Dos endpoints de income** — NO son duplicados: `/api/sivra/income` = listado completo (JOIN a `properties`, página `/sivra/income`); `/api/sivra/incomes` = filtrado por fechas para el calendario. Distintos usos.
