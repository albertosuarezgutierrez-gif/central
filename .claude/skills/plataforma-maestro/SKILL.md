...
| **Panel de agentes** (god-panel → 🤖 Agentes, `/operador/agentes`, PR #797, 09/07/2026) | Lista TODOS los agentes del monorepo (rutinas Claude, Agente Director, crons agénticos de Vercel) desde el catálogo tipado `lib/agentes-catalogo.ts` (fuente única en código), con salud en vivo por semáforo 🟢🟡🔴/⚪ (`lib/agentes-salud.ts`, última actividad en BD vs cadencia esperada). Esquema completo en `docs/AGENTES-MAPA.md`. **Director — 4 funciones** vía filtro puro `lib/director-modelos.ts::modelosPermitidos` (estrecha el catálogo ANTES de decidir): F1 degradación gradual por presupuesto (`ratioPresupuestoDiario`, ver `DIRECTOR_PRESUPUESTO_UMBRAL` arriba); F2 enrutado por contexto + preferencia `eu` (RGPD); F3 núcleo reutilizable `lib/pasarela.ts::chatConDirector` (`/api/ai/chat` es wrapper fino; el agente contable ya enruta por el Director vía `CONTABLE_MODEL`); F4 bucle de aprendizaje determinista en `ia-director-refresh` (penaliza modelos con mala racha, versiona en `ia_director_aprendizaje`). |
