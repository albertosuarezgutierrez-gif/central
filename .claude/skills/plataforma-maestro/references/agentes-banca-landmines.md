...
- **Correduría `/correduria`** (`app/(usuario)/correduria/`; **ya no está en el sidebar** desde el
  ...
  - **🛡️ `/correduria` YA NO es solo la matriz de comisiones (01/09/2026):** debajo pinta la **cartera
    VIVA** del CRM de Manuel y las **renovaciones a 90 días**, leídas por el puerto HTTP de
    central-asegura (`lib/cartera-asegura.ts` → `/api/correduria/{cartera,vencimientos}` → asegura
    `/api/operador/{resumen,vencimientos}`, Bearer `ASEGURA_OPERADOR_SECRET`). **No mezcles las dos
    mitades en un informe:** arriba es dinero YA COBRADO (comisiones sobre `movimientos_bancarios`),
    abajo son pólizas/clientes. Tres estados en todo el camino: `sin_configurar` (el puerto no está
    conectado) ≠ `error` (con MOTIVO: `secreto_rechazado`/`asegura_error`/`respuesta_ilegible`/`red`)
    ≠ `ok` con lista vacía. Una cartera a ceros NUNCA se pinta por un fallo de red.
  - **🚗 Columna «Qué asegura» de las renovaciones (PR #1938, 01/09/2026):** el objeto asegurado sale
    del helper PURO `@central/module-seguros/objeto::objetoAsegurado` con **cuatro** salidas —
    `conocido` · `no_informado` (la compañía no lo manda) · `cifrado` (la dirección de hogar llega
    `v1:iv:cipher:tag`, AES-256-GCM, y `PII_ENCRYPTION_KEY` vive en el Vercel de Manuel) ·
    `sin_objeto` (vida/salud/decesos aseguran PERSONAS: ausencia definitiva, no «pendiente»). Aguas
    abajo hay un 5º estado: `objeto === null` = «la versión desplegada del puerto aún no manda el
    campo», que se arregla desplegando, no reclamándole el dato a la compañía. **Ninguno se pinta
    como celda vacía.** ⚠️ `datos_especificos.vehiculo` **contiene la MATRÍCULA**, no una descripción
    (2.781 filas): pintarla como modelo es inventarse el dato. Una RC no tiene bien — la identifican
    sus modalidades en `poliza_coberturas`, que por eso se consultan SOLO para los ramos que las
    necesitan (un auto trae 25 coberturas que no dicen nada del coche).
