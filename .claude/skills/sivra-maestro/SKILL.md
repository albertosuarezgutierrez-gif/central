...
- **Prisma** con conexión directa (`DATABASE_URL`/`DIRECT_URL`); auth NextAuth v5 (admin) + cookie
  `limpiadora_token`. IA por `lib/ai-client.ts` → **pasarela central de plataforma** (`aiComplete`+`aiExtractInvoice` OCR; envs `AI_GATEWAY_URL`+`AI_GATEWAY_SECRET`, fallback NIM directo; `seo-refresh` sigue Anthropic directo). Deploy Vercel, 10 crons en `vercel.json`.
