...
- **seguros (correduría, deducible):** compañías de seguros (Generali, Allianz, Mapfre, Caser…), **CABIFY** (desplazamientos de la correduría — el recibo llega de `no-reply@mgx.cabify.com` con asunto `Alberto, tu viaje por X €`; incluye origen/destino/importe), **GOOGLE** (Google Workspace, Google One, Google Drive, suscripciones de Google usadas para el negocio — factura/recibo de Google o PayPal a Google), y el **software/IA de desarrollo** (decisión de Alberto 03/07/2026): **ANTHROPIC/CLAUDE** (recibo vía Stripe `invoice+statements+acct_1REyrSBNUnCSzfs9@stripe.com`, asunto «Your receipt from Anthropic Ireland»), **VERCEL** (hosting, `invoice+statements@vercel.com`, «Your receipt from Vercel Inc.»), **fal.ai** (IA, `noreply@fal.ai` / `withorb.com`) y **OEPM** (tasas de registro de marca «casa de marcas», `no.responder@oepm.es` con PDF adjunto). ⚠️ Casi todos se pagan con la **tarjeta N26** (`iban='N26-MANUAL'`), que NO está conectada por PSD2 → sus cargos se importan a mano; y **parte de la suscripción de Claude está en la cuenta de Manuel Suárez** (`info@manuelsuarez.es`, no conectado), factura a nombre de Alberto pero recibo en su buzón → pedírselo.
