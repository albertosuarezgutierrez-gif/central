...
- **Aviso Telegram**: como esta skill corre en una sesión Claude (no en el runtime de plataforma), NO uses
  ...
  <ALERTA_SECRET>` (token de alertas del entorno; `CRON_SECRET` de respaldo) y `{ "mensaje": "🔴 Extracción de facturas caída N días · M en cola · revisa la QUERY del Apps Script (allowlist, NO OAuth)" }`
